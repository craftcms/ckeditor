import { ImageInsertUI as Bc, ButtonView as Yo, IconImage as Hc, Command as Os, Plugin as Vn, ImageUtils as Jl, Collection as qo, ViewModel as Ko, createDropdown as ki, DropdownButtonView as Wc, IconObjectSizeMedium as $c, addListToDropdown as Ei, Widget as Yc, viewToModelPositionOutsideModelElement as qc, toWidget as Kc, DomEventObserver as Qc, View as Gr, IconPlus as ec, WidgetToolbarRepository as Gl, isWidget as Gc, findAttributeRange as Xc, LinkUI as Xl, ContextualBalloon as Zc, Range as Jc, SwitchButtonView as eu, LabeledFieldView as tu, createLabeledInputText as nu, ClassicEditor as ru, SourceEditing as tc, Heading as ou } from "ckeditor5";
/**
...
class Nu extends Bc {
  ...
  _createToolbarImageButton(R) {
    const m = this.editor, i = m.t, c = new Yo(R);
    c.isEnabled = !0, c.label = i("Insert image"), c.icon = Hc, c.tooltip = !0;
    const u = m.commands.get("insertImage");
    return c.bind("isEnabled").to(u), this.listenTo(c, "execute", () => this._showImageSelectModal()), c;
  }
  _showImageSelectModal() {
    const R = this._assetSources, m = this.editor, i = m.config, c = Object.assign({}, i.get("assetSelectionCriteria"), {
      kind: "image"
    });
  ...
  _processAssetUrls(R, m) {
    return new Promise((i) => {
      ...
      const c = this.editor, u = c.config.get("defaultTransform"), r = new Craft.Queue(), _ = [];
      r.on("afterRun", () => {
        c.execute("insertImage", { source: _ }), i();
      });
      for (const g of R)
        r.push(
          () => new Promise((b) => {
            ...
            if (!v && u)
              this._getTransformUrl(g.id, u, (y) => {
                _.push(y), b();
              });
            else {
  ...
  _getTransformUrl(R, m, i) {
    Craft.sendActionRequest("POST", "ckeditor/ckeditor/image-url", {
      ...
      i(this._buildAssetUrl(R, c.url, m));
    }).catch(() => {
...
class iu extends Os {
   ...
   */
  execute(R) {
    const i = this.editor.model, c = this._element(), u = this._srcInfo(c);
    if (this.value = {
      transform: R.transform
    }, u) {
      const r = `#asset:${u.assetId}` + (R.transform ? `:transform:${R.transform}` : "");
      i.change((_) => {
        const g = u.src.replace(/#.*/, "") + r;
        _.setAttribute("src", g, c);
      }), Craft.sendActionRequest("post", "ckeditor/ckeditor/image-url", {
        data: {
          assetId: u.assetId,
          transform: R.transform
        ...
        i.change((g) => {
          const b = _.url + r;
          g.setAttribute("src", b, c), _.width && g.setAttribute("width", _.width, c), _.height && g.setAttribute("height", _.height, c);
        });
      });
    }
  }
...
class nc extends Vn {
  ...
  init() {
    const R = this.editor, m = new iu(R);
...
class su extends Vn {
  ...
  _registerImageTransformDropdown(R) {
    const m = this.editor, i = m.t, c = {
      ...
      const g = m.commands.get("transformImage"), b = ki(_, Wc), v = b.buttonView;
      return v.set({
        tooltip: i("Resize image"),
        commandValue: null,
        icon: au,
        isToggleable: !0,
        label: this._getOptionLabelValue(c),
        withText: !0,
        class: "ck-resize-image-button"
      }), v.bind("label").to(g, "value", (y) => {
        if (!y || !y.transform)
          return this._getOptionLabelValue(c);
        const C = R.find(
        ...
        b,
        () => this._getTransformDropdownListItemDefinitions(u, g),
        {
          ariaLabel: i("Image resize list")
        }
      ), this.listenTo(b, "execute", (y) => {
        m.execute(y.source.commandName, {
          transform: y.source.commandValue
        }), m.editing.view.focus();
      }), b;
    };
    m.ui.componentFactory.add("transformImage", r);
  }
  /**
    ...
    return R.map((c) => {
      const u = {
        ...
        model: new Ko({
          ...
          commandValue: c.value,
          label: this._getOptionLabelValue(c),
          withText: !0,
          icon: null
        })
      };
      u.model.bind("isOn").to(m, "value", lu(c.value)), i.add(u);
    }), i;
  }
}
function lu(xe) {
...
class cu extends Os {
  refresh() {
    ...
    if (this.isEnabled = !!m, this.isEnabled) {
      ...
      Craft.sendActionRequest("POST", "ckeditor/ckeditor/image-permissions", {
        ...
        c.data.editable === !1 && (this.isEnabled = !1);
      });
    }
  }
  /**
  ...
  execute() {
    ...
    if (i) {
      ...
      new Craft.AssetImageEditor(i.assetId, c);
    }
  }
  /**
...
class rc extends Vn {
  ...
  init() {
    const R = this.editor, m = new cu(R);
...
class uu extends Vn {
  ...
  _registerImageEditorButton() {
    const R = this.editor, m = R.t, i = R.commands.get("imageEditor"), c = () => {
      const u = new Yo();
      return u.set({
        label: m("Edit Image"),
        withText: !0
    ...
    R.ui.componentFactory.add("imageEditor", c);
  }
}
/**
 * @link https://craftcms.com/
...
class du extends Os {
  execute(R) {
    ...
    m.model.change((c) => {
      const u = c.createElement("craftEntryModel", {
        ...Object.fromEntries(i.getAttributes()),
        cardHtml: R.cardHtml,
        entryId: R.entryId,
        siteId: R.siteId
      });
      m.model.insertObject(u, null, null, {
        setSelection: "on"
      });
    });
  }
...
class fu extends Vn {
  ...
  _defineConverters() {
    ...
    R.for("upcast").elementToElement({
      ...
      model: (i, { writer: c }) => {
        ...
        return c.createElement("craftEntryModel", {
          cardHtml: u,
          entryId: r,
          siteId: _
        });
      ...
      view: (i, { writer: c }) => {
        const u = i.getAttribute("entryId") ?? null, r = i.getAttribute("siteId") ?? null, _ = c.createContainerElement("div", {
          class: "cke-entry-card",
          "data-entry-id": u,
          "data-site-id": r
        });
        return m(i, c, _), Kc(_, c);
      }
    }), R.for("dataDowncast").elementToElement({
      model: "craftEntryModel",
      view: (i, { writer: c }) => {
        const u = i.getAttribute("entryId") ?? null, r = i.getAttribute("siteId") ?? null;
        return c.createContainerElement("craft-entry", {
          "data-entry-id": u,
          "data-site-id": r
        });
      }
    ...
    const m = (i, c, u) => {
      this._getCardHtml(i).then((r) => {
        ...
        c.insert(c.createPositionAt(u, 0), _);
        const g = this.editor;
  ...
  async _getCardHtml(R) {
    ...
    const c = $(i[0]).data("layout-element");
    if (m)
      return { cardHtml: m };
    const u = R.getAttribute("entryId") ?? null, r = R.getAttribute("siteId") ?? null;
    try {
...
class hu extends Gr {
  constructor(R, m = {}) {
    super(R), this.bindTemplate, this.set("isFocused", !1), this.entriesUi = m.entriesUi, this.editor = this.entriesUi.editor;
    const i = this.editor.commands.get("insertEntry");
    let c = new qo(), u = new qo();
    if (this.entriesUi._getEntryTypeButtonsCollection(m.entryTypeOptions ?? []).forEach((r, _) => {
      let g = new Yo();
      if (r.model.icon) {
...
class mu extends Vn {
   ...
   */
  init() {
    this._createToolbarEntriesButtons(), this.editor.ui.componentFactory.add("editEntryBtn", (R) => this._createEditEntryBtn(R)), this._listenToEvents();
  }
  /**
   * @inheritDoc
   */
  ...
  _listenToEvents() {
    ...
    R.addObserver(pu), this.editor.listenTo(m, "dblclick", (i, c) => {
      if (!this.editor.isReadOnly) {
        const u = this.editor.editing.mapper.toModelElement(
          c.target.parent
        );
        u.name === "craftEntryModel" && this._initEditEntrySlideout(c, u);
      }
    });
  }
  _initEditEntrySlideout(R = null, m = null) {
    ...
    const i = m.getAttribute("entryId"), c = m.getAttribute("siteId") ?? null;
    this._showEditEntrySlideout(i, c, m);
  }
  /**
  ...
  _createSingleToolbarEntriesButton(R) {
    const m = this.editor, i = m.config.get("entryTypeOptions"), c = m.commands.get("insertEntry");
    if (!i || !i.length)
      return;
    const u = ki(R);
  ...
  _createToolbarEntriesButtons() {
    const R = this.editor, m = R.config.get("entryTypeOptions");
    if (!m || !m.length)
      return;
    ...
    ) : this.editor.ui.componentFactory.add("createEntry", (c) => this._createSingleToolbarEntriesButton(c));
  }
  /**
  ...
  _showEditEntrySlideout(R, m, i) {
    const c = this.editor, u = c.model, r = this.getElementEditor();
    ...
    const g = _.data("owner-id"), b = Craft.createElementEditor(this.elementType, null, {
      ...
      onBeforeSubmit: async () => {
        if (_ !== null && Garnish.hasAttr(_, "data-owner-is-canonical") && (!r || !r.settings.isUnpublishedDraft)) {
          ...
          let v = $(c.sourceElement).attr("name");
      ...
      onSubmit: (v) => {
        let y = this._getCardElement(R);
        y !== null && v.data.id != y.data("id") && (y.attr("data-id", v.data.id).data("id", v.data.id).data("owner-id", v.data.ownerId), c.editing.model.change((C) => {
          C.setAttribute("entryId", v.data.id, i), c.ui.update();
        }), Craft.refreshElementInstances(v.data.id));
      }
    });
    b.on("beforeClose", () => {
      u.change((v) => {
        v.setSelection(v.createPositionAfter(i)), c.editing.view.focus();
      });
    }), b.on("close", () => {
      c.editing.view.focus();
    });
  }
  /**
  ...
  async _showCreateEntrySlideout(R) {
    var y, C;
    const m = this.editor, i = m.model, u = i.document.selection.getFirstRange(), r = m.config.get(
      "nestedElementAttributes"
    ), _ = Object.assign({}, r, {
      typeId: R
    }), g = this.getElementEditor();
    g && (await g.markDeltaNameAsModified(m.sourceElement.name), _.ownerId = g.getDraftElementId(
      r.ownerId
    ));
    let b;
    try {
...
class bu extends Vn {
  ...
  _defineConverters() {
    ...
    for (let m = 0; m < this.conversionData.length; m++)
      R.for("downcast").attributeToElement({
        ...
        view: (i, { writer: c }) => {
          const u = c.createAttributeElement(
            "a",
            { [this.conversionData[m].view]: i },
            { priority: 5 }
          );
          return c.setCustomProperty("link", !0, u), u;
        }
        ...
        model: {
          ...
          value: (i, c) => i.getAttribute(this.conversionData[m].view)
        }
      });
  }
  _adjustLinkCommand() {
    ...
    m.on(
      ...
      (c, u) => {
        ...
        c.stop(), i = !0;
        const r = u[u.length - 1], _ = R.model.document.selection;
        R.model.change((g) => {
          R.execute("link", ...u);
  ...
  _adjustUnlinkCommand() {
    const R = this.editor, m = R.commands.get("unlink"), { model: i } = R, { selection: c } = i.document;
    let u = !1;
    m.on(
      "execute",
      (r) => {
        u || (r.stop(), i.change(() => {
          u = !0, R.execute("unlink"), u = !1, i.change((_) => {
            let g;
...
class yu extends Gr {
  constructor(R, m = {}) {
    ...
    if (this.button = null, i) {
      const c = this.linkUi.linkTypeDropdownItemModels[i];
      this.linkUi.linkTypeDropdownView.buttonView.label == c.label && (this.button = Craft.t("app", "Loading"));
    }
    this.button == null && (this.button = new Yo(), this.button.set({
  ...
  render() {
    ...
    this.element.addEventListener("click", function(c) {
      (this.children[0].classList.contains("add") || c.target.classList.contains("ck-button__label")) && (m._hideUI(), R._showElementSelectorModal(i));
      ...
      var u;
      if (Object.keys(c.data.elements).length > 0) {
        for (const [g, b] of Object.entries(
          this.linkUi.sitesView.siteDropdownItemModels
        ))
          c.data.siteIds.includes(parseInt(g)) || g == "current" ? b.set("isEnabled", !0) : b.set("isEnabled", !1);
        this.element.innerHTML = c.data.elements[this.elementId][0], Craft.appendHeadHtml(c.data.headHtml), Craft.appendBodyHtml(c.data.bodyHtml);
        ...
        Craft.addActionsToChip(r, _), R._alignFocus();
        ...
        this.button = new Yo(), this.button.set({
          label: Craft.t("app", "Choose"),
          withText: !0,
          class: "btn add icon dashed"
      ...
      var u, r, _, g;
      throw Craft.cp.displayError((r = (u = c == null ? void 0 : c.response) == null ? void 0 : u.data) == null ? void 0 : r.message), ((g = (_ = c == null ? void 0 : c.response) == null ? void 0 : _.data) == null ? void 0 : g.message) ?? c;
    });
  }
}
class vu extends Gr {
  constructor(R, m = {}) {
    super(R), this.bindTemplate, this.set("isFocused", !1), this.linkUi = m.linkUi, this.editor = this.linkUi.editor, this.elementId = this.linkUi._getLinkElementId(), this.siteId = this.linkUi._getLinkSiteId(), this.linkOption = m.linkOption, this.linkUi._getLinkElementRefHandle(), this.siteDropdownView = ki(this.linkUi._linkUI.formView.locale), this.siteDropdownItemModels = null, this.localizedRefHandleRE = null;
  ...
  _sitesDropdown() {
    ...
    this.siteDropdownView.buttonView.set({
      ...
      new qo([
        ...Craft.sites.map((c) => ({
          type: "button",
          model: this.siteDropdownItemModels[c.id]
        })),
        {
          type: "button",
    ...
    ), this.siteDropdownView.on("execute", (c) => {
      ...
      const { siteId: r } = c.source;
      let _ = u[1];
      r && (_ += `@${r}`), this.linkUi.previousLinkValue = this.linkUi._urlInputValue();
      const g = this.linkUi._urlInputValue().replace(u[0], _);
  ...
  _selectSiteDropdownItem(R) {
    ...
    this.siteDropdownView.buttonView.set("label", i), Object.values(this.siteDropdownItemModels).forEach((c) => {
      c.set("isOn", c.siteId === m.siteId);
    });
  }
}
class wu extends Gr {
  constructor(R, m = {}) {
    ...
    }), this.summary = new Gr(R), this.summary.setTemplate({
      tag: "summary",
      attributes: {
        class: ["ck", "ck-form__details__summary"]
      },
      children: [{ text: i.to("label") }]
    }), this.children.add(this.summary), this.advancedFieldsContainer = new Gr(R), this.advancedFieldsContainer.setTemplate({
      tag: "div",
      attributes: {
  ...
  onToggle(R) {
    ...
    if (R.target.open) {
      ...
      this.advancedChildren._items.forEach((c, u) => {
        m._focusables.add(c, i + u + 1), m.focusTracker.add(c.element, i + u + 1);
      });
    } else
      this.advancedChildren._items.forEach((i, c) => {
...
class ku extends Vn {
  ...
  _linkOptionsDropdown() {
    ...
    this.linkTypeDropdownView = ki(R.locale), this.linkTypeDropdownView.buttonView.set({
      label: "",
      withText: !0,
      isVisible: !0
    }), this.linkTypeDropdownItemModels = Object.fromEntries(
      this._getLinkListItemDefinitions().map((c) => [c.handle, c])
    ), Ei(
      this.linkTypeDropdownView,
      new qo([
        ...this._getLinkListItemDefinitions().map((c) => ({
          type: "button",
          model: this.linkTypeDropdownItemModels[c.handle]
        }))
      ])
    ), i.isEmpty && this._showLinkTypeForm("default"), this.linkTypeDropdownView.on("execute", (c) => {
      if (c.source.linkOption) {
        const u = c.source.linkOption;
        this._selectLinkTypeDropdownItem(u.refHandle), this._showLinkTypeForm(u, R);
      } else
        this._selectLinkTypeDropdownItem("default"), this._showLinkTypeForm("default");
    }), this.listenTo(i, "change:value", () => {
      this._toggleLinkTypeDropdownView();
      const c = this._getLinkElementRefHandle();
  ...
  _selectLinkTypeDropdownItem(R) {
    ...
    this.linkTypeDropdownView.buttonView.set("label", i), Object.values(this.linkTypeDropdownItemModels).forEach((c) => {
      c.set("isOn", c.handle === m.handle);
    });
  }
  /**
  ...
  _showLinkTypeForm(R) {
    var _, g, b, v;
    const { formView: m } = this._linkUI, { children: i } = m, { urlInputView: c } = m, { displayedTextInputView: u } = m;
    u.focus(), this.linkTypeWrapperView !== null && i.remove(this.linkTypeWrapperView), R === "default" ? (this.elementInputView = c, this.sitesView !== null && (g = (_ = this.sitesView) == null ? void 0 : _.siteDropdownView) != null && g.buttonView && this.sitesView.siteDropdownView.buttonView.set("isVisible", !1)) : (this.elementInputView = new yu(m.locale, {
      linkUi: this,
      linkOption: R,
      value: this._urlInputValue()
    }), this.sitesView !== null && (v = (b = this.sitesView) == null ? void 0 : b.siteDropdownView) != null && v.buttonView && this.sitesView.siteDropdownView.buttonView.set("isVisible", !0)), Craft.isMultiSite && this.sitesView == null && (this.sitesView = new vu(m.locale, {
      linkUi: this,
      linkOption: R
    }));
    let r = new Gr();
  ...
  _showElementSelectorModal(R) {
    const m = this.editor, i = m.model, c = i.document.selection, u = c.isCollapsed, r = c.getFirstRange(), _ = this._linkUI._getSelectedLinkElement(), g = () => {
      m.editing.view.focus(), !u && r && i.change((b) => {
        b.setSelection(r);
  ...
  _addAdvancedLinkFieldInputs() {
    ...
    this.advancedView = new wu(m.locale, {
      linkUi: this
    }), i.add(this.advancedView, 3);
    for (const u of this.advancedLinkFields) {
      let r = (c = u.conversion) == null ? void 0 : c.model;
      if (r && typeof m[r] > "u")
        if (u.conversion.type === "bool") {
          ...
          _.set({
            ...
            label: u.label,
            isToggleable: !0
          }), u.tooltip && (_.tooltip = u.tooltip), this.advancedView.advancedChildren.add(_), m[r] = _, m[r].bind("isOn").to(R, r, (g) => g === void 0 ? (m[r].element.value = "", !1) : (m[r].element.value = u.conversion.value, !0)), _.on("execute", () => {
            _.isOn ? (_.isOn = !1, m[r].element.value = "") : (_.isOn = !0, m[r].element.value = u.conversion.value);
          });
        } else {
          let _ = this._addLabeledField(u);
          m[r] = _, m[r].fieldView.bind("value").to(R, r), m[r].fieldView.element.value = R[r] || "";
        }
  ...
  _toggleUrlSuffixInputView(R, m) {
    if (m)
      ...
      let c = null;
      i ? c = i[1] : c = this._urlInputValue();
      try {
        let u = new URL(c), r = u.search, _ = u.hash;
        R.fieldView.set("value", r + _);
      } catch {
        let [r, _] = c.split("#"), [g, b] = r.split("?");
  ...
  _handleAdvancedLinkFieldsFormSubmit() {
    ...
    i.on(
      "submit",
      () => {
        let c = this._getAdvancedFieldValues();
        m.once(
          "execute",
          (u, r) => {
            r.length === 4 ? Object.assign(r[3], c) : r.push(c);
          },
          { priority: "highest" }
        );
      },
  ...
  _trackAdvancedLinkFieldsValueChange() {
    ...
    this.conversionData.forEach((c) => {
      m.set(c.model, null), R.model.document.on("change", () => {
        m[c.model] = i.getAttribute(c.model);
      });
    });
  }
  /**
  ...
  _getAdvancedFieldValues() {
    ...
    return this.conversionData.forEach((i) => {
      let c = [];
      i.type === "bool" ? c[i.model] = R[i.model].element.value : c[i.model] = R[i.model].fieldView.element.value, Object.assign(m, c);
    }), m;
  }
}
class Iu extends Vn {
    ...
    (function(m, i) {
      ...
      return function(m) {
        ...
        function c(u) {
          ...
          return m[u].call(r.exports, r, r.exports, c), r.l = !0, r.exports;
        }
        return c.m = m, c.c = i, c.d = function(u, r, _) {
          c.o(u, r) || Object.defineProperty(u, r, { enumerable: !0, get: _ });
        }, c.r = function(u) {
          ...
          if (1 & r && (u = c(u)), 8 & r || 4 & r && typeof u == "object" && u && u.__esModule) return u;
          var _ = /* @__PURE__ */ Object.create(null);
          if (c.r(_), Object.defineProperty(_, "default", { enumerable: !0, value: u }), 2 & r && typeof u != "string") for (var g in u) c.d(_, g, (function(b) {
            return u[b];
          }).bind(null, g));
          return _;
        }, c.n = function(u) {
        ...
        function r(v, y = !0) {
          ...
          const C = Object(u.stringify)(v, b, null, { maxDepth: 2 });
        ...
        function x(N) {
          return u(N) ? r(N) ? "attribute:" + N.name : y(N) ? "root:" + N.name : "container:" + N.name : N.data;
        }
        c.d(i, "d", function() {
          return u;
        }), c.d(i, "b", function() {
        ...
        (function(u, r) {
          ...
          var A, ae, le, ne = Array.prototype, ce = Function.prototype, ye = Object.prototype, ee = K["__core-js_shared__"], de = ce.toString, D = ye.hasOwnProperty, ie = (A = /[^.]+$/.exec(ee && ee.keys && ee.keys.IE_PROTO || "")) ? "Symbol(src)_1." + A : "", be = ye.toString, Te = RegExp("^" + de.call(D).replace(/[\\^$.*+?()[\]{}|]/g, "\\$&").replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g, "$1.*?") + "$"), ke = P ? K.Buffer : void 0, Pe = K.Symbol, Se = K.Uint8Array, ze = ye.propertyIsEnumerable, Je = ne.splice, X = Pe ? Pe.toStringTag : void 0, Y = Object.getOwnPropertySymbols, me = ke ? ke.isBuffer : void 0, l = (ae = Object.keys, le = Object, function(W) {
            return ae(le(W));
          ...
          function pt(W, se) {
            var we = ar(W), Ce = !we && ir(W), it = !we && !Ce && Cn(W), qe = !we && !Ce && !it && Xr(W), st = we || Ce || it || qe, tt = st ? function(bt, Pt) {
          ...
          function Sn(W) {
            return W == null ? W === void 0 ? "[object Undefined]" : "[object Null]" : X && X in Object(W) ? function(se) {
              ...
              var qe = be.call(se);
              return it && (we ? se[X] = Ce : delete se[X]), qe;
            }(W) : function(se) {
              return be.call(se);
            }(W);
          }
          ...
          function _r(W, se, we, Ce, it) {
            return W === se || (W == null || se == null || !On(W) && !On(se) ? W != W && se != se : function(qe, st, tt, Ot, nt, bt) {
              var Pt = ar(qe), tn = ar(st), ht = Pt ? "[object Array]" : ut(qe), Bt = tn ? "[object Array]" : ut(st), Pn = (ht = ht == _ ? b : ht) == b, ct = (Bt = Bt == _ ? b : Bt) == b, yn = ht == Bt;
              if (yn && Cn(qe)) {
                if (!Cn(st)) return !1;
                Pt = !0, Pn = !1;
              }
              if (yn && !Pn) return bt || (bt = new Tt()), Pt || Xr(qe) ? Yt(qe, st, tt, Ot, nt, bt) : function(rt, Ue, qn, Kt, Or, At, nn) {
                switch (qn) {
              ...
              if (!(1 & tt)) {
                ...
                if (un || Nn) {
                  var _o = un ? qe.value() : qe, xo = Nn ? st.value() : st;
          ...
          function Eo(W) {
            return !(!lr(W) || function(se) {
              return !!ie && ie in se;
            }(W)) && (sr(W) ? Te : y).test(bn(W));
          }
          function en(W) {
            if (we = (se = W) && se.constructor, Ce = typeof we == "function" && we.prototype || ye, se !== Ce) return l(W);
            var se, we, Ce, it = [];
            for (var qe in Object(W)) D.call(W, qe) && qe != "constructor" && it.push(qe);
            return it;
          }
          function Yt(W, se, we, Ce, it, qe) {
            ...
            for (qe.set(W, se), qe.set(se, W); ++bt < tt; ) {
              var ht = W[bt], Bt = se[bt];
              if (Ce) var Pn = st ? Ce(Bt, ht, bt, se, W, qe) : Ce(ht, Bt, bt, W, se, qe);
              if (Pn !== void 0) {
                if (Pn) continue;
                Pt = !1;
                break;
            ...
            return qe.delete(W), qe.delete(se), Pt;
          }
          function xr(W) {
          ...
          kt.prototype.clear = function() {
            ...
            return D.call(se, W) ? se[W] : void 0;
          }, kt.prototype.has = function(W) {
            var se = this.__data__;
            return he ? se[W] !== void 0 : D.call(se, W);
            ...
            var se = this.__data__, we = Yn(se, W);
          ...
          (f && ut(new f(new ArrayBuffer(1))) != "[object DataView]" || k && ut(new k()) != g || U && ut(U.resolve()) != "[object Promise]" || F && ut(new F()) != v || H && ut(new H()) != "[object WeakMap]") && (ut = function(W) {
...
`);
        ...
        function _(v) {
          ...
          if (typeof v[0] == "string") u.a.warn(`[CKEditorInspector] The CKEditorInspector.attach( '${v[0]}', editor ) syntax has been deprecated and will be removed in the near future. To pass a name of an editor instance, use CKEditorInspector.attach( { '${v[0]}': editor } ) instead. Learn more in https://github.com/ckeditor/ckeditor5-inspector/blob/master/README.md.`), y.editors[v[0]] = v[1];
          else {
        ...
        function N(P) {
          ...
          if (Object(u.d)(P)) {
            Object(u.g)(P) ? (M.type = "RootEditableElement", M.name = P.rootName, M.url = g + "_rooteditableelement-RootEditableElement.html") : (M.name = P.name, Object(u.b)(P) ? (M.type = "AttributeElement", M.url = g + "_attributeelement-AttributeElement.html") : Object(u.e)(P) ? (M.type = "EmptyElement", M.url = g + "_emptyelement-EmptyElement.html") : Object(u.h)(P) ? (M.type = "UIElement", M.url = g + "_uielement-UIElement.html") : Object(u.f)(P) ? (M.type = "RawElement", M.url = g + "_rawelement-RawElement.html") : Object(u.c)(P) ? (M.type = "EditableElement", M.url = g + "_editableelement-EditableElement.html") : (M.type = "ContainerElement", M.url = g + "_containerelement-ContainerElement.html")), V(P).forEach(([B, I]) => {
        ...
        function N(M, B) {
          ...
          Object(u.c)(B) ? (Object(u.d)(B) ? (I.type = "RootElement", I.name = B.rootName, I.url = g + "rootelement-RootElement.html") : (I.type = "Element", I.name = B.name, I.url = g + "element-Element.html"), I.properties = { childCount: { value: B.childCount }, startOffset: { value: B.startOffset }, endOffset: { value: B.endOffset }, maxOffset: { value: B.maxOffset } }) : (I.name = B.data, I.type = "Text", I.url = g + "text-Text.html", I.properties = { startOffset: { value: B.startOffset }, endOffset: { value: B.endOffset }, offsetSize: { value: B.offsetSize } }), I.properties.path = { value: Object(u.b)(B) }, j(B).forEach(([J, Q]) => {
              ...
              for (const ce of ne) {
                ...
                for (const ee of ye) {
                  ...
                  if (de === 0) {
                    ...
                    D ? D.positionsBefore.push(ee) : le.positions.push(ee);
                  } else if (de === le.maxOffset) {
                    const D = le.children[le.children.length - 1];
                    D ? D.positionsAfter.push(ee) : le.positions.push(ee);
                  } else {
                    let D = ee.isEnd ? 0 : le.children.length - 1, ie = le.children[D];
                    for (; ie; ) {
                      ...
                      D += ee.isEnd ? 1 : -1, ie = le.children[D];
                    }
                  }
                }
              }
        ...
        var u = c(0), r = c.n(u), _ = c(5), g = c.n(_);
...
`, "\\n"], ["\f", "\\f"], ["\r", "\\r"], ["'", "\\'"], ['"', '\\"'], ["\\", "\\\\"]]);
        ...
        function _(y, C) {
          return r(y, this, y.length === 1 ? c : u, C.cache.create(), C.serializer);
        }
        function g() {
...
`) : C;
        ...
        const u = c(25), r = c(13), _ = Symbol("root");
...
` : "";
        ...
        Object.defineProperty(i, "__esModule", { value: !0 }), i.findInArray = function(u, r) {
          for (var _ = 0, g = u.length; _ < g; _++) if (r.apply(r, [u[_], _, u])) return u[_];
        ...
        function ne(X, Y, me) {
          ...
          if (Y != null) for (l in Y.ref !== void 0 && (U = Y.ref), Y.key !== void 0 && (k = "" + Y.key), Y) ae.call(Y, l) && !le.hasOwnProperty(l) && (f[l] = Y[l]);
        ...
        i.Children = { map: function(X, Y, me) {
          ...
          return X;
        } }, i.Component = I, i.Fragment = b, i.Profiler = y, i.PureComponent = Q, i.StrictMode = v, i.Suspense = G, i.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED = Je, i.cloneElement = function(X, Y, me) {
          if (X == null) throw Error(P(267, X));
          var l = u({}, X.props), f = X.key, k = X.ref, U = X._owner;
          if (Y != null) {
            if (Y.ref !== void 0 && (k = Y.ref, U = A.current), Y.key !== void 0 && (f = "" + Y.key), X.type && X.type.defaultProps) var F = X.type.defaultProps;
            for (H in Y) ae.call(Y, H) && !le.hasOwnProperty(H) && (l[H] = Y[H] === void 0 && F !== void 0 ? F[H] : Y[H]);
          }
          ...
          return { $$typeof: _, type: X.type, key: f, ref: k, props: l, _owner: U };
        ...
        var u = c(0), r = c(16), _ = c(23);
        function g(e) {
          for (var t = "https://reactjs.org/docs/error-decoder.html?invariant=" + e, n = 1; n < arguments.length; n++) t += "&args[]=" + encodeURIComponent(arguments[n]);
          return "Minified React error #" + e + "; visit " + t + " for the full message or use the non-minified dev environment for full errors and additional helpful warnings.";
        }
        if (!u) throw Error(g(227));
        function b(e, t, n, o, a, p, w, T, Z) {
            ...
            if (!Q[n]) {
              ...
              for (var o in Q[n] = t, n = t.eventTypes) {
                ...
                if (Z) {
                  ...
                  a = !0;
                } else p.registrationName ? (J(p.registrationName, w, T), a = !0) : a = !1;
        ...
        function k(e, t, n, o, a, p) {
          this.acceptsBooleans = t === 2 || t === 3 || t === 4, this.attributeName = o, this.attributeNamespace = a, this.mustUseProperty = n, this.propertyName = e, this.type = t, this.sanitizeURL = p;
        }
        var U = {};
        ...
        function je(e, t, n, o) {
          ...
          (a !== null ? a.type === 0 : !o && 2 < t.length && (t[0] === "o" || t[0] === "O") && (t[1] === "n" || t[1] === "N")) || (function(p, w, T, Z) {
            ...
            return !1;
          }(t, n, a, o) && (n = null), o || a === null ? function(p) {
          ...
          do {
            e: switch (e.tag) {
              ...
              case 9:
                ...
                break e;
              default:
                var o = e._debugOwner, a = e._debugSource, p = Yt(e.type);
                n = null, o && (n = Yt(o.type)), o = p, p = "", a ? p = " (at " + a.fileName.replace(Re, "") + ":" + a.lineNumber + ")" : n && (p = " (created by " + n + ")"), n = `
    in ` + (o || "Unknown") + p;
            }
            t += n, e = e.return;
          } while (e);
        ...
        function bn(e, t) {
          ...
          n = qt(t.value != null ? t.value : n), e._wrapperState = { initialChecked: o, initialValue: n, controlled: t.type === "checkbox" || t.type === "radio" ? t.checked != null : t.value != null };
        }
        function Tr(e, t) {
        ...
        function Xr(e, t) {
          var n = qt(t.value), o = qt(t.defaultValue);
        ...
        function rn(e) {
          ...
          for (n = 0; n < e.ancestors.length; n++) {
            ...
            for (var T = null, Z = 0; Z < Q.length; Z++) {
              var q = Q[Z];
              q && (q = q.extractEvents(o, t, p, a, w)) && (T = Ue(T, q));
            }
            At(T);
          }
        }
        ...
        function Nr(e, t, n, o, a, p) {
          return e === null || e.nativeEvent !== p ? (e = So(t, n, o, a, p), t !== null && (t = io(t)) !== null && ur(t), e) : (e.eventSystemFlags |= o, e);
        }
        function Oa(e) {
          var t = Mr(e.target);
          if (t !== null) {
        ...
        function Qo(e, t) {
          for (var n = 0; n < e.length; n += 2) {
            var o = e[n], a = e[n + 1], p = "on" + (a[0].toUpperCase() + a.slice(1));
            p = { phasedRegistrationNames: { bubbled: p, captured: p + "Capture" }, dependencies: [o], eventPriority: t }, Rr.set(o, t), Ci.set(o, p), Si[a] = p;
          }
        }
          ...
          if (To) if (0 < Ut.length && -1 < Bn.indexOf(e)) e = So(null, e, t, n, o), Ut.push(e);
            ...
            if (a === null) wn(e, o);
              ...
              switch (w) {
                ...
                case "gotpointercapture":
                  return ge = q.pointerId, Jr.set(ge, Nr(Jr.get(ge) || null, p, w, T, Z, q)), !0;
              }
              return !1;
            }(a, e, t, n, o)) {
        ...
        function Xo(e, t) {
          if (t) {
            if (Ni[e] && (t.children != null || t.dangerouslySetInnerHTML != null)) throw Error(g(137, e, ""));
        ...
        function Fi() {
          if (Ro) return Ro;
          var e, t, n = ai, o = n.length, a = "value" in hr ? hr.value : hr.textContent, p = a.length;
          for (e = 0; e < o && n[e] === a[e]; e++) ;
          var w = o - e;
          for (t = 1; t <= w && n[o - t] === a[p - t]; t++) ;
        ...
        var Ie = !1, Ke = { eventTypes: pe, extractEvents: function(e, t, n, o) {
          ...
          return p ? (re && n.locale !== "ko" && (Ie || p !== pe.compositionStart ? p === pe.compositionEnd && Ie && (a = Fi()) : (ai = "value" in (hr = o) ? hr.value : hr.textContent, Ie = !0)), p = s.getPooled(p, t, n, o), (a || (a = Be(n)) !== null) && (p.data = a), jr(p), a = p) : a = null, (e = L ? function(w, T) {
            switch (w) {
              case "compositionend":
                return Be(T);
              case "keypress":
        ...
        var dn = 0, Xn = 0, Gt = !1, fn = !1, Lr = xt.extend({ screenX: null, screenY: null, clientX: null, clientY: null, pageX: null, pageY: null, ctrlKey: null, shiftKey: null, altKey: null, metaKey: null, getModifierState: $t, button: null, buttons: null, relatedTarget: function(e) {
          ...
          if (p && (32 & a) == 0 && (n.relatedTarget || n.fromElement) || !w && !p || (p = o.window === o ? o : (p = o.ownerDocument) ? p.defaultView || p.parentWindow : window, w ? (w = t, (t = (t = n.relatedTarget || n.toElement) ? Mr(t) : null) !== null && (t !== Nn(t) || t.tag !== 5 && t.tag !== 6) && (t = null)) : w = null, w === t)) return null;
          if (e === "mouseout" || e === "mouseover") var T = Lr, Z = Ur.mouseLeave, q = Ur.mouseEnter, ge = "mouse";
          else e !== "pointerout" && e !== "pointerover" || (T = si, Z = Ur.pointerLeave, q = Ur.pointerEnter, ge = "pointer");
          if (e = w == null ? p : Gn(w), p = t == null ? p : Gn(t), (Z = T.getPooled(Z, w, n, o)).type = ge + "leave", Z.target = e, Z.relatedTarget = p, (n = T.getPooled(q, t, n, o)).type = ge + "enter", n.target = p, n.relatedTarget = e, ge = t, (o = w) && ge) e: {
            for (q = ge, w = 0, e = T = o; e; e = Rn(e)) w++;
          ...
          for (q = T, T = []; o && o !== q && ((w = o.alternate) === null || w !== q); ) T.push(o), o = Rn(o);
          for (o = []; ge && ge !== q && ((w = ge.alternate) === null || w !== q); ) o.push(ge), ge = Rn(ge);
        ...
        var oc = { eventTypes: li, extractEvents: function(e, t, n, o, a, p) {
          if (!(p = !(a = p || (o.window === o ? o.document : o.nodeType === 9 ? o : o.ownerDocument)))) {
            e: {
        ...
        function Lo(e, t) {
          ...
          var a, p = {};
          for (a in n) p[a] = t[a];
          return o && ((e = e.stateNode).__reactInternalMemoizedUnmaskedChildContext = t, e.__reactInternalMemoizedMaskedChildContext = p), p;
        }
        function hn(e) {
        ...
        function Ds(e, t, n) {
          ...
          for (var a in o = o.getChildContext()) if (!(a in e)) throw Error(g(108, Yt(t) || "Unknown", a));
          return r({}, n, {}, o);
        }
        function $i(e) {
        ...
        var gc = _.unstable_runWithPriority, za = _.unstable_scheduleCallback, Is = _.unstable_cancelCallback, As = _.unstable_requestPaint, La = _.unstable_now, bc = _.unstable_getCurrentPriorityLevel, Yi = _.unstable_ImmediatePriority, Ms = _.unstable_UserBlockingPriority, js = _.unstable_NormalPriority, zs = _.unstable_LowPriority, Ls = _.unstable_IdlePriority, Us = {}, yc = _.unstable_shouldYield, vc = As !== void 0 ? As : function() {
        }, br = null, qi = null, Ua = !1, Fs = La(), jn = 1e4 > Fs ? La : function() {
          return La() - Fs;
        };
        function Ki() {
          switch (bc()) {
        ...
        function Hs(e) {
          return br === null ? (br = [e], qi = za(Yi, Ws)) : br.push(e), Us;
        }
        function Jn() {
          if (qi !== null) {
            var e = qi;
        ...
        function Ws() {
          if (!Ua && br !== null) {
            ...
            try {
              ...
              throw br !== null && (br = br.slice(e + 1)), za(Yi, Jn), n;
            } finally {
              Ua = !1;
            }
          }
        ...
        function ci(e, t, n, o) {
          ...
          if (p !== null) {
            ...
            He === null ? ge = Z : He.next = De, a.baseState = ge, a.baseQueue = He, xa(q), e.expirationTime = q, e.memoizedState = Z;
          }
        }
        function qs(e, t, n) {
        ...
        function Qs(e, t, n, o, a, p, w) {
          return typeof (e = e.stateNode).shouldComponentUpdate == "function" ? e.shouldComponentUpdate(o, p, w) : !t.prototype || !t.prototype.isPureReactComponent || !gr(n, o) || !gr(a, p);
        }
        function Gs(e, t, n) {
          var o = !1, a = Fr, p = t.contextType;
          return typeof p == "object" && p !== null ? p = zn(p) : (a = hn(t) ? co : Xt.current, p = (o = (o = t.contextTypes) != null) ? Lo(e, a) : Fr), t = new t(n, p), e.memoizedState = t.state !== null && t.state !== void 0 ? t.state : null, t.updater = ea, e.stateNode = t, t._reactInternalFiber = e, o && ((e = e.stateNode).__reactInternalMemoizedUnmaskedChildContext = a, e.__reactInternalMemoizedMaskedChildContext = p), t;
        }
        function Xs(e, t, n, o) {
        ...
        function Wa(e, t, n, o) {
          ...
          var p = t.contextType;
          typeof p == "object" && p !== null ? a.context = zn(p) : (p = hn(t) ? co : Xt.current, a.context = Lo(e, p)), ci(e, n, a, o), a.state = e.memoizedState, typeof (p = t.getDerivedStateFromProps) == "function" && (Ji(e, t, p, n), a.state = e.memoizedState), typeof t.getDerivedStateFromProps == "function" || typeof a.getSnapshotBeforeUpdate == "function" || typeof a.UNSAFE_componentWillMount != "function" && typeof a.componentWillMount != "function" || (t = a.state, typeof a.componentWillMount == "function" && a.componentWillMount(), typeof a.UNSAFE_componentWillMount == "function" && a.UNSAFE_componentWillMount(), t !== a.state && ea.enqueueReplaceState(a, a.state, null), ci(e, n, a, o), a.state = e.memoizedState), typeof a.componentDidMount == "function" && (e.effectTag |= 4);
        }
          ...
          if ((e = n.ref) !== null && typeof e != "function" && typeof e != "object") {
            if (n._owner) {
              ...
              return t !== null && t.ref !== null && typeof t.ref == "function" && t.ref._stringRef === a ? t.ref : ((t = function(p) {
                var w = o.refs;
                w === Ks && (w = o.refs = {}), p === null ? delete w[a] : w[a] = p;
              })._stringRef = a, t);
            }
        ...
        function Zs(e) {
          ...
          function Fn(oe, te, ue, ve) {
            for (var _e = null, Oe = null, Ve = te, at = te = 0, Rt = null; Ve !== null && at < ue.length; at++) {
              ...
              e && Ve && Ze.alternate === null && t(oe, Ve), te = p(Ze, te, at), Oe === null ? _e = Ze : Oe.sibling = Ze, Oe = Ze, Ve = Rt;
            }
            if (at === ue.length) return n(oe, Ve), _e;
            if (Ve === null) {
              for (; at < ue.length; at++) (Ve = De(oe, ue[at], ve)) !== null && (te = p(Ve, te, at), Oe === null ? _e = Ve : Oe.sibling = Ve, Oe = Ve);
              return _e;
            }
            for (Ve = o(oe, Ve); at < ue.length; at++) (Rt = ot(Ve, oe, at, ue[at], ve)) !== null && (e && Rt.alternate !== null && Ve.delete(Rt.key === null ? at : Rt.key), te = p(Rt, te, at), Oe === null ? _e = Rt : Oe.sibling = Rt, Oe = Rt);
          ...
          function ln(oe, te, ue, ve) {
            ...
            for (var Oe = _e = null, Ve = te, at = te = 0, Rt = null, Ze = ue.next(); Ve !== null && !Ze.done; at++, Ze = ue.next()) {
              ...
              e && Ve && Vt.alternate === null && t(oe, Ve), te = p(Vt, te, at), Oe === null ? _e = Vt : Oe.sibling = Vt, Oe = Vt, Ve = Rt;
            }
            ...
            for (Ve = o(oe, Ve); !Ze.done; at++, Ze = ue.next()) (Ze = ot(Ve, oe, at, Ze.value, ve)) !== null && (e && Ze.alternate !== null && Ve.delete(Ze.key === null ? at : Ze.key), te = p(Ze, te, at), Oe === null ? _e = Ze : Oe.sibling = Ze, Oe = Ze);
        ...
        function Ga(e, t, n, o, a, p) {
          if ($r = p, Lt = t, t.memoizedState = null, t.updateQueue = null, t.expirationTime = 0, oa.current = e === null || e.memoizedState === null ? wc : kc, e = n(o, a), t.expirationTime === $r) {
            p = 0;
            do {
              if (t.expirationTime = 0, !(25 > p)) throw Error(g(301));
              p += 1, Zt = sn = null, t.updateQueue = null, oa.current = Ec, e = n(o, a);
            } while (t.expirationTime === $r);
          }
          if (oa.current = ua, t = sn !== null && sn.next !== null, $r = 0, Zt = sn = Lt = null, ia = !1, t) throw Error(g(300));
          return e;
        }
        function Ho() {
        ...
        function aa(e) {
          ...
          var o = sn, a = o.baseQueue, p = n.pending;
          if (p !== null) {
            if (a !== null) {
              var w = a.next;
              a.next = p.next, p.next = w;
            }
            o.baseQueue = a = p, n.pending = null;
          }
          if (a !== null) {
            ...
            do {
              ...
              if (q < $r) {
                ...
                T === null ? (w = T = ge, p = o) : T = T.next = ge, q > Lt.expirationTime && (Lt.expirationTime = q, xa(q));
              } else T !== null && (T = T.next = { expirationTime: 1073741823, suspenseConfig: Z.suspenseConfig, action: Z.action, eagerReducer: Z.eagerReducer, eagerState: Z.eagerState, next: null }), Vl(q, Z.suspenseConfig), o = Z.eagerReducer === e ? Z.eagerState : e(o, Z.action);
              Z = Z.next;
            } while (Z !== null && Z !== a);
            T === null ? p = o : T.next = w, mr(o, t.memoizedState) || (tr = !0), t.memoizedState = o, t.baseState = p, t.baseQueue = T, n.lastRenderedState = o;
          }
        ...
        function sa(e) {
          ...
          if (n === null) throw Error(g(311));
          n.lastRenderedReducer = e;
          var o = n.dispatch, a = n.pending, p = t.memoizedState;
          if (a !== null) {
            n.pending = null;
            var w = a = a.next;
            do
        ...
        function es(e, t, n, o) {
          ...
          if (sn !== null) {
            ...
            if (p = w.destroy, o !== null && Qa(o, w.deps)) return void Za(t, n, p, o);
          }
          Lt.effectTag |= e, a.memoizedState = Za(1 | t, n, p, o);
        }
        function tl(e, t) {
          return Ja(516, 4, e, t);
        }
        ...
        function sl(e, t, n) {
          ...
          var p = t.pending;
          if (p === null ? a.next = a : (a.next = p.next, p.next = a), t.pending = a, p = e.alternate, e === Lt || p !== null && p === Lt) ia = !0, a.expirationTime = $r, Lt.expirationTime = $r;
          else {
        ...
        function da(e) {
          ...
          var t = e.type;
          if (e.tag !== 5 || t !== "head" && t !== "body" && !ri(t, e.memoizedProps)) for (t = Yr; t; ) ll(e, t), t = Ar(t.nextSibling);
          if (ul(e), e.tag === 13) {
        ...
        function dl(e, t, n, o, a) {
          n = n.render;
          var p = t.ref;
          return Fo(t, a), o = Ga(e, t, n, o, p, a), e === null || tr ? (t.effectTag |= 1, Un(e, t, o, a), t.child) : (t.updateQueue = e.updateQueue, t.effectTag &= -517, e.expirationTime <= a && (e.expirationTime = 0), vr(e, t, a));
        }
        function fl(e, t, n, o, a, p) {
          if (e === null) {
            var w = n.type;
            return typeof w != "function" || vs(w) || w.defaultProps !== void 0 || n.compare !== null || n.defaultProps !== void 0 ? ((e = Sa(n.type, null, o, null, t.mode, p)).ref = t.ref, e.return = t, t.child = e) : (t.tag = 15, t.type = w, pl(e, t, w, o, a, p));
          }
          return w = e.child, a < p && (a = w.memoizedProps, (n = (n = n.compare) !== null ? n : gr)(a, o) && e.ref === t.ref) ? vr(e, t, p) : (t.effectTag |= 1, (e = wo(w, o)).ref = t.ref, e.return = t, t.child = e);
        }
        function pl(e, t, n, o, a, p) {
          return e !== null && gr(e.memoizedProps, o) && e.ref === t.ref && (tr = !1, a < p) ? (t.expirationTime = e.expirationTime, vr(e, t, p)) : is(e, t, n, o, p);
        }
        function hl(e, t) {
          var n = t.ref;
        ...
        function is(e, t, n, o, a) {
          var p = hn(n) ? co : Xt.current;
          return p = Lo(t, p), Fo(t, a), n = Ga(e, t, n, o, p, a), e === null || tr ? (t.effectTag |= 1, Un(e, t, n, a), t.child) : (t.updateQueue = e.updateQueue, t.effectTag &= -517, e.expirationTime <= a && (e.expirationTime = 0), vr(e, t, a));
        }
        function ml(e, t, n, o, a) {
          ...
          if (Fo(t, a), t.stateNode === null) e !== null && (e.alternate = null, t.alternate = null, t.effectTag |= 2), Gs(t, n, o), Wa(t, n, o, a), o = !0;
            ...
            typeof q == "object" && q !== null ? q = zn(q) : q = Lo(t, q = hn(n) ? co : Xt.current);
            var ge = n.getDerivedStateFromProps, De = typeof ge == "function" || typeof w.getSnapshotBeforeUpdate == "function";
            De || typeof w.UNSAFE_componentWillReceiveProps != "function" && typeof w.componentWillReceiveProps != "function" || (T !== o || Z !== q) && Xs(t, w, o, q), Br = !1;
            var He = t.memoizedState;
            w.state = He, ci(t, o, w, a), Z = t.memoizedState, T !== o || He !== Z || pn.current || Br ? (typeof ge == "function" && (Ji(t, n, ge, o), Z = t.memoizedState), (T = Br || Qs(t, n, T, o, He, Z, q)) ? (De || typeof w.UNSAFE_componentWillMount != "function" && typeof w.componentWillMount != "function" || (typeof w.componentWillMount == "function" && w.componentWillMount(), typeof w.UNSAFE_componentWillMount == "function" && w.UNSAFE_componentWillMount()), typeof w.componentDidMount == "function" && (t.effectTag |= 4)) : (typeof w.componentDidMount == "function" && (t.effectTag |= 4), t.memoizedProps = o, t.memoizedState = Z), w.props = o, w.state = Z, w.context = q, o = T) : (typeof w.componentDidMount == "function" && (t.effectTag |= 4), o = !1);
          } else w = t.stateNode, Ha(e, t), T = t.memoizedProps, w.props = t.type === t.elementType ? T : $n(t.type, T), Z = w.context, typeof (q = n.contextType) == "object" && q !== null ? q = zn(q) : q = Lo(t, q = hn(n) ? co : Xt.current), (De = typeof (ge = n.getDerivedStateFromProps) == "function" || typeof w.getSnapshotBeforeUpdate == "function") || typeof w.UNSAFE_componentWillReceiveProps != "function" && typeof w.componentWillReceiveProps != "function" || (T !== o || Z !== q) && Xs(t, w, o, q), Br = !1, Z = t.memoizedState, w.state = Z, ci(t, o, w, a), He = t.memoizedState, T !== o || Z !== He || pn.current || Br ? (typeof ge == "function" && (Ji(t, n, ge, o), He = t.memoizedState), (ge = Br || Qs(t, n, T, o, Z, He, q)) ? (De || typeof w.UNSAFE_componentWillUpdate != "function" && typeof w.componentWillUpdate != "function" || (typeof w.componentWillUpdate == "function" && w.componentWillUpdate(o, He, q), typeof w.UNSAFE_componentWillUpdate == "function" && w.UNSAFE_componentWillUpdate(o, He, q)), typeof w.componentDidUpdate == "function" && (t.effectTag |= 4), typeof w.getSnapshotBeforeUpdate == "function" && (t.effectTag |= 256)) : (typeof w.componentDidUpdate != "function" || T === e.memoizedProps && Z === e.memoizedState || (t.effectTag |= 4), typeof w.getSnapshotBeforeUpdate != "function" || T === e.memoizedProps && Z === e.memoizedState || (t.effectTag |= 256), t.memoizedProps = o, t.memoizedState = He), w.props = o, w.state = He, w.context = q, o = ge) : (typeof w.componentDidUpdate != "function" || T === e.memoizedProps && Z === e.memoizedState || (t.effectTag |= 4), typeof w.getSnapshotBeforeUpdate != "function" || T === e.memoizedProps && Z === e.memoizedState || (t.effectTag |= 256), o = !1);
          return as(e, t, n, o, p, a);
        }
        function as(e, t, n, o, a, p) {
          hl(e, t);
          var w = (64 & t.effectTag) != 0;
          if (!o && !w) return a && Rs(t, n, !1), vr(e, t, p);
          o = t.stateNode, _c.current = t;
          var T = w && typeof n.getDerivedStateFromError != "function" ? null : o.render();
          return t.effectTag |= 1, e !== null && w ? (t.child = Vo(t, e.child, null, p), t.child = Vo(t, null, T, p)) : Un(e, t, T, p), t.memoizedState = o.state, a && Rs(t, n, !0), t.child;
        }
        function gl(e) {
          var t = e.stateNode;
        ...
        function wl(e, t, n) {
          var o, a = t.mode, p = t.pendingProps, w = Ct.current, T = !1;
          if ((o = (64 & t.effectTag) != 0) || (o = (2 & w) != 0 && (e === null || e.memoizedState !== null)), o ? (T = !0, t.effectTag &= -65) : e !== null && e.memoizedState === null || p.fallback === void 0 || p.unstable_avoidThisFallback === !0 || (w |= 1), St(Ct, 1 & w), e === null) {
          ...
          if (e.memoizedState !== null) {
            if (a = (e = e.child).sibling, T) {
              if (p = p.fallback, (n = wo(e, e.pendingProps)).return = t, (2 & t.mode) == 0 && (T = t.memoizedState !== null ? t.child.child : t.child) !== e.child) for (n.child = T; T !== null; ) T.return = n, T = T.sibling;
              return (a = wo(a, p)).return = t, n.sibling = a, n.childExpirationTime = 0, t.memoizedState = ss, t.child = n, a;
            }
        ...
        function ls(e, t, n, o, a, p) {
          var w = e.memoizedState;
          w === null ? e.memoizedState = { isBackwards: t, rendering: null, renderingStartTime: 0, last: o, tail: n, tailExpiration: 0, tailMode: a, lastEffect: p } : (w.isBackwards = t, w.rendering = null, w.renderingStartTime = 0, w.last = o, w.tail = n, w.tailExpiration = 0, w.tailMode = a, w.lastEffect = p);
        }
        function El(e, t, n) {
          var o = t.pendingProps, a = o.revealOrder, p = o.tail;
        ...
        function xc(e, t, n) {
          ...
          switch (t.tag) {
                ...
                if (e = uo(er.current), da(t)) {
                  ...
                  switch (o[Qn] = t, o[ro] = p, a) {
                    ...
                    case "details":
                      ...
                      break;
                    case "input":
                      bn(o, p), mt("invalid", o), Hn(n, "onChange");
                      break;
                    case "select":
                      o._wrapperState = { wasMultiple: !!p.multiple }, mt("invalid", o), Hn(n, "onChange");
                      break;
                    case "textarea":
                      On(o, p), mt("invalid", o), Hn(n, "onChange");
                  }
                  for (var w in Xo(a, p), e = null, p) if (p.hasOwnProperty(w)) {
                    var T = p[w];
                  ...
                  switch (a) {
                    ...
                    case "option":
                      break;
                    default:
                      typeof p.onClick == "function" && (o.onclick = Po);
                  }
                  ...
                  for (p in Z) if (Z.hasOwnProperty(p)) {
                    var q = Z[p];
                    p === "style" ? Pi(e, q) : p === "dangerouslySetInnerHTML" ? (q = q ? q.__html : void 0) != null && qe(e, q) : p === "children" ? typeof q == "string" ? (a !== "textarea" || q !== "") && st(e, q) : typeof q == "number" && st(e, "" + q) : p !== "suppressContentEditableWarning" && p !== "suppressHydrationWarning" && p !== "autoFocus" && (A.hasOwnProperty(p) ? q != null && Hn(n, p) : q != null && je(e, p, q, w));
                  }
                  switch (a) {
                    case "input":
                      Sr(e), ar(e, o, !1);
                    ...
                    case "textarea":
                      ...
                      break;
                    case "option":
                      o.value != null && e.setAttribute("value", "" + qt(o.value));
                      break;
                    case "select":
            ...
            case 6:
              ...
              return null;
            case 13:
              return vt(Ct), o = t.memoizedState, (64 & t.effectTag) != 0 ? (t.expirationTime = n, t) : (n = o !== null, o = !1, e === null ? t.memoizedProps.fallback !== void 0 && da(t) : (o = (a = e.memoizedState) !== null, n || a === null || (a = e.child.sibling) !== null && ((p = t.firstEffect) !== null ? (t.firstEffect = a, a.nextEffect = p) : (t.firstEffect = t.lastEffect = a, a.nextEffect = null), a.effectTag = 8)), n && !o && (2 & t.mode) != 0 && (e === null && t.memoizedProps.unstable_avoidThisFallback !== !0 || (1 & Ct.current) != 0 ? Ft === ho && (Ft = ha) : (Ft !== ho && Ft !== ha || (Ft = ma), gi !== 0 && _n !== null && (ko(_n, mn), Yl(_n, gi)))), (n || o) && (t.effectTag |= 4), null);
            case 4:
              return Bo(), null;
            case 10:
              return Va(t), null;
            ...
            case 19:
              ...
              if (a = (64 & t.effectTag) != 0, (p = o.rendering) === null) {
                ...
                if (!a) if ((e = ra(p)) !== null) {
                  if (t.effectTag |= 64, a = !0, (n = e.updateQueue) !== null && (t.updateQueue = n, t.effectTag |= 4), fa(o, !0), o.tail === null && o.tailMode === "hidden" && !p.alternate) return (t = t.lastEffect = o.lastEffect) !== null && (t.nextEffect = null), null;
                } else 2 * jn() - o.renderingStartTime > o.tailExpiration && 1 < n && (t.effectTag |= 64, a = !0, fa(o, !1), t.expirationTime = t.childExpirationTime = n - 1);
                o.isBackwards ? (p.sibling = t.child, t.child = p) : ((n = o.last) !== null ? n.sibling = p : t.child = p, o.last = p);
              }
          ...
          if (p !== o) {
            ...
            switch (uo(er.current), e = null, n) {
              ...
              case "option":
                ...
                break;
              case "select":
                p = r({}, p, { value: void 0 }), o = r({}, o, { value: void 0 }), e = [];
                break;
              case "textarea":
                p = lr(Z, p), o = lr(Z, o), e = [];
                break;
              default:
                typeof p.onClick != "function" && typeof o.onClick == "function" && (Z.onclick = Po);
            }
            for (w in Xo(n, o), n = null, p) if (!o.hasOwnProperty(w) && p.hasOwnProperty(w) && p[w] != null) if (w === "style") for (T in Z = p[w]) Z.hasOwnProperty(T) && (n || (n = {}), n[T] = "");
            ...
            for (w in o) {
              var q = o[w];
              if (Z = p != null ? p[w] : void 0, o.hasOwnProperty(w) && q !== Z && (q != null || Z != null)) if (w === "style") if (Z) {
        ...
        function us(e, t) {
          ...
          o === null && n !== null && (o = xr(n)), n !== null && Yt(n.type), t = t.value, e !== null && e.tag === 1 && Yt(e.type);
          try {
            console.error(t);
          } catch (a) {
            setTimeout(function() {
        ...
        function Nl(e, t, n) {
          for (var o, a, p = t, w = !1; ; ) {
            ...
            if (p.tag === 5 || p.tag === 6) {
              ...
              a ? (T = o, Z = p.stateNode, T.nodeType === 8 ? T.parentNode.removeChild(Z) : T.removeChild(Z)) : o.removeChild(p.stateNode);
            } else if (p.tag === 4) {
              if (p.child !== null) {
                o = p.stateNode.containerInfo, a = !0, p.child.return = p, p = p.child;
                continue;
              }
          ...
          switch (t.tag) {
            ...
            case 5:
              ...
              if (n != null) {
                ...
                if (t.updateQueue = null, p !== null) {
                  for (n[ro] = o, e === "input" && o.type === "radio" && o.name != null && Tr(n, o), Zo(e, a), t = Zo(e, o), a = 0; a < p.length; a += 2) {
                    var w = p[a], T = p[a + 1];
                    w === "style" ? Pi(n, T) : w === "dangerouslySetInnerHTML" ? qe(n, T) : w === "children" ? st(n, T) : je(n, w, T, t);
                  }
                  switch (e) {
                    case "input":
                      ir(n, o);
            ...
            case 13:
              if (n = t, t.memoizedState === null ? o = !1 : (o = !0, n = t.child, fs = jn()), n !== null) e: for (e = n; ; ) {
                if (e.tag === 5) p = e.stateNode, o ? typeof (p = p.style).setProperty == "function" ? p.setProperty("display", "none", "important") : p.display = "none" : (p = e.stateNode, a = (a = e.memoizedProps.style) != null && a.hasOwnProperty("display") ? a.display : null, p.style.display = Oi("display", a));
        ...
        function Il(e, t, n) {
          ...
          return p !== null && typeof p.componentDidCatch == "function" && (n.callback = function() {
            typeof o != "function" && (qr === null ? qr = /* @__PURE__ */ new Set([this]) : qr.add(this), us(e, t));
            var w = t.stack;
            this.componentDidCatch(t.value, { componentStack: w !== null ? w : "" });
          }), n;
        }
        var Al, Nc = Math.ceil, pa = he.ReactCurrentDispatcher, Ml = he.ReactCurrentOwner, ho = 0, ha = 3, ma = 4, Qe = 0, _n = null, Ge = null, mn = 0, Ft = ho, ga = null, wr = 1073741823, mi = 1073741823, ba = null, gi = 0, ya = !1, fs = 0, Ae = null, va = !1, ps = null, qr = null, wa = !1, bi = null, yi = 90, mo = null, vi = 0, hs = null, ka = 0;
        function nr() {
        ...
        function Kr(e, t) {
          if (50 < vi) throw vi = 0, hs = null, Error(g(185));
          if ((e = Ea(e, t)) !== null) {
            var n = Ki();
        ...
        function Ea(e, t) {
          ...
          return a !== null && (_n === a && (xa(t), Ft === ma && ko(a, mn)), Yl(a, t)), a;
        }
        function _a(e) {
              ...
              if (Ge === null) switch (a = e.finishedWork = e.current.alternate, e.finishedExpirationTime = n, o = Ft, _n = null, o) {
                ...
                case 5:
                  if (wr !== 1073741823 && ba !== null) {
                    p = wr;
                    var w = ba;
                    if (0 >= (o = 0 | w.busyMinDurationMs) ? o = 0 : (a = 0 | w.busyDelayMs, o = (p = jn() - (10 * (1073741821 - p) - (0 | w.timeoutMs || 5e3))) <= a ? 0 : a + o - p), 10 < o) {
...
Add a <Suspense fallback=...> component higher in the tree to provide a loading indicator or placeholder to display.` + xr(w));
            ...
            if (e = Ge.return, (2048 & Ge.effectTag) == 0) {
              if (t = xc(t, Ge, mn), mn === 1 || Ge.childExpirationTime !== 1) {
                for (var n = 0, o = Ge.child; o !== null; ) {
                  var a = o.expirationTime, p = o.childExpirationTime;
                  a > n && (n = a), p > n && (n = p), o = o.sibling;
                }
        ...
        function Ic(e, t) {
            ...
            if (ei(w)) {
              if ("selectionStart" in w) var T = { start: w.selectionStart, end: w.selectionEnd };
                ...
                if (Z && Z.rangeCount !== 0) {
                  ...
                  t: for (; ; ) {
                    for (var ue; oe !== T || q !== 0 && oe.nodeType !== 3 || (He = De + q), oe !== ge || Z !== 0 && oe.nodeType !== 3 || (ot = De + Z), oe.nodeType === 3 && (De += oe.nodeValue.length), (ue = oe.firstChild) !== null; ) te = oe, oe = ue;
                    for (; ; ) {
                      if (oe === w) break t;
                      if (te === T && ++Fn === q && (He = De), te === ge && ++ln === Z && (ot = De), (ue = oe.nextSibling) !== null) break;
                      te = (oe = te).parentNode;
                    }
                    oe = ue;
                  }
          ...
          if ((t = e.firstPendingTime) === 0 && (qr = null), t === 1073741823 ? e === hs ? vi++ : (vi = 0, hs = e) : vi = 0, typeof bs == "function" && bs(n.stateNode, o), xn(e), va) throw va = !1, e = ps, ps = null, e;
        ...
        function Mc() {
          if (bi === null) return !1;
          var e = bi;
          if (bi = null, (48 & Qe) != 0) throw Error(g(331));
          var t = Qe;
          ...
          switch (t.expirationTime = 0, t.tag) {
            case 2:
              if (o = t.type, e !== null && (e.alternate = null, t.alternate = null, t.effectTag |= 2), e = t.pendingProps, a = Lo(t, Xt.current), Fo(t, n), a = Ga(null, t, o, e, a, n), t.effectTag |= 1, typeof a == "object" && a !== null && typeof a.render == "function" && a.$$typeof === void 0) {
                ...
                typeof w == "function" && Ji(t, o, w, e), a.updater = ea, t.stateNode = a, a._reactInternalFiber = t, Wa(t, o, e, n), t = as(null, t, o, !0, p, n);
            ...
            case 3:
              ...
              if (o = t.pendingProps, a = (a = t.memoizedState) !== null ? a.element : null, Ha(e, t), ci(t, o, null, n), (o = t.memoizedState.element) === a) os(), t = vr(e, t, n);
                ...
                if ((a = t.stateNode.hydrate) && (Yr = Ar(t.stateNode.containerInfo.firstChild), yr = t, a = po = !0), a) for (n = $a(t, null, o, n), t.child = n; n; ) n.effectTag = -3 & n.effectTag | 1024, n = n.sibling;
                else Un(e, t, o, n), os();
                t = t.child;
              }
              return t;
            case 5:
              return Js(t), e === null && rs(t), o = t.type, a = t.pendingProps, p = e !== null ? e.memoizedProps : null, w = a.children, ri(o, a) ? w = null : p !== null && ri(o, p) && (t.effectTag |= 16), hl(e, t), 4 & t.mode && n !== 1 && a.hidden ? (t.expirationTime = t.childExpirationTime = 1, t = null) : (Un(e, t, w, n), t = t.child), t;
            case 6:
              return e === null && rs(t), null;
            case 13:
              return wl(e, t, n);
            case 4:
              return Ya(t, t.stateNode.containerInfo), o = t.pendingProps, e === null ? t.child = Vo(t, null, o, n) : Un(e, t, o, n), t.child;
            case 11:
            ...
            case 10:
              e: {
                o = t.type._context, a = t.pendingProps, w = t.memoizedProps, p = a.value;
                var T = t.type._context;
                if (St(Gi, T._currentValue), T._currentValue = p, w !== null) if (T = w.value, (p = mr(T, p) ? 0 : 0 | (typeof o._calculateChangedBits == "function" ? o._calculateChangedBits(T, p) : 1073741823)) === 0) {
                  ...
                  if (Z !== null) {
                    ...
                    for (var q = Z.firstContext; q !== null; ) {
                      if (q.context === o && (q.observedBits & p) != 0) {
              ...
              return t;
            case 9:
              return a = t.type, o = (p = t.pendingProps).children, Fo(t, n), o = o(a = zn(a, p.unstable_observedBits)), t.effectTag |= 1, Un(e, t, o, n), t.child;
            case 14:
              return p = $n(a = t.type, t.pendingProps), fl(e, t, a, p = $n(a.type, p), o, n);
            case 15:
        ...
        function Sa(e, t, n, o, a, p) {
          ...
          return (t = rr(w, n, t, a)).elementType = e, t.type = o, t.expirationTime = p, t;
        }
        function Qr(e, t, n, o) {
        ...
        function Ca(e, t, n, o) {
          var a = t.current, p = nr(), w = ui.suspense;
          p = go(p, a, w);
          e: if (n) {
            t: {
          ...
          return t.context === null ? t.context = n : t.pendingContext = n, (t = Hr(p, w)).payload = { element: e }, (o = o === void 0 ? null : o) !== null && (t.callback = o), Wr(a, t), Kr(a, p), p;
        }
        function _s(e) {
        ...
        function Ss(e, t, n) {
          ...
          o.current = a, a.stateNode = o, Ba(a), e[oo] = o.current, n && t !== 0 && function(p, w) {
            var T = un(w);
        ...
        function Ta(e, t, n, o, a) {
          ...
          if (p) {
            ...
            }(n, o), w = p._internalRoot, typeof a == "function") {
              var Z = a;
              a = function() {
                var q = _s(w);
                Z.call(q);
        ...
        if (typeof window > "u" || typeof MessageChannel != "function") {
          ...
          i.unstable_now = function() {
            ...
            v !== null ? setTimeout(u, 0, Y) : (v = Y, setTimeout(C, 0));
          }, r = function(Y, me) {
            y = setTimeout(Y, me);
          }, _ = function() {
            clearTimeout(y);
          ...
          g = function() {
            ...
            0 > Y || 125 < Y ? console.error("forceFrameRate takes a positive int between 0 and 125, forcing framerates higher than 125 fps is not unsupported") : J = 0 < Y ? Math.floor(1e3 / Y) : 5;
          };
        ...
        var ye = [], ee = [], de = 1, D = null, ie = 3, be = !1, Te = !1, ke = !1;
        function Pe(Y) {
        ...
        function Se(Y) {
          if (ke = !1, Pe(Y), !Te) if (le(ye) !== null) Te = !0, u(ze);
          else {
            var me = le(ee);
            me !== null && r(Se, me.startTime - Y);
          }
        }
        function ze(Y, me) {
          Te = !1, ke && (ke = !1, _()), be = !0;
          ...
          try {
            for (Pe(me), D = le(ye); D !== null && (!(D.expirationTime > me) || Y && !g()); ) {
              ...
              if (f !== null) {
                ...
                me = i.unstable_now(), typeof k == "function" ? D.callback = k : D === le(ye) && ne(ye), Pe(me);
              } else ne(ye);
              D = le(ye);
            }
            if (D !== null) var U = !0;
        ...
        i.unstable_IdlePriority = 5, i.unstable_ImmediatePriority = 1, i.unstable_LowPriority = 4, i.unstable_NormalPriority = 3, i.unstable_Profiling = null, i.unstable_UserBlockingPriority = 2, i.unstable_cancelCallback = function(Y) {
          ...
          return Y = { id: de++, callback: me, priorityLevel: Y, startTime: k, expirationTime: l = k + l, sortIndex: -1 }, k > f ? (Y.sortIndex = k, ae(ee, Y), le(ye) === null && Y === le(ee) && (ke ? _() : ke = !0, r(Se, k - f))) : (Y.sortIndex = l, ae(ye, Y), Te || be || (Te = !0, u(ze))), Y;
        ...
        const u = c(13), r = c(26), _ = c(17), g = { string: u.quoteString, number: (b) => Object.is(b, -0) ? "-0" : String(b), boolean: String, symbol: (b, v, y) => {
...
` + x);
        ...
        }).call(this, c(27).Buffer, c(15));
      }, function(m, i, c) {
        (function(u) {
          var r = c(28), _ = c(29), g = c(30);
          function b() {
          ...
          function y(l, f, k) {
            if (!(y.TYPED_ARRAY_SUPPORT || this instanceof y)) return new y(l, f, k);
            if (typeof l == "number") {
          ...
          function N(l, f) {
            if (x(f), l = v(l, f < 0 ? 0 : 0 | K(f)), !y.TYPED_ARRAY_SUPPORT) for (var k = 0; k < f; ++k) l[k] = 0;
            return l;
          }
          function G(l, f) {
          ...
          function M(l, f, k, U, F) {
            ...
            if (typeof f == "string" && (f = y.from(f, U)), y.isBuffer(f)) return f.length === 0 ? -1 : B(l, f, k, U, F);
            if (typeof f == "number") return f &= 255, y.TYPED_ARRAY_SUPPORT && typeof Uint8Array.prototype.indexOf == "function" ? F ? Uint8Array.prototype.indexOf.call(l, f, k) : Uint8Array.prototype.lastIndexOf.call(l, f, k) : B(l, [f], k, U, F);
          ...
          function le(l, f, k) {
            return f === 0 && k === l.length ? r.fromByteArray(l) : r.fromByteArray(l.slice(f, k));
          }
          function ne(l, f, k) {
          ...
          i.Buffer = y, i.SlowBuffer = function(l) {
          ...
          }, i.INSPECT_MAX_BYTES = 50, y.TYPED_ARRAY_SUPPORT = u.TYPED_ARRAY_SUPPORT !== void 0 ? u.TYPED_ARRAY_SUPPORT : function() {
            try {
          ...
          }, y.from = function(l, f, k) {
            return C(null, l, f, k);
          }, y.TYPED_ARRAY_SUPPORT && (y.prototype.__proto__ = Uint8Array.prototype, y.__proto__ = Uint8Array, typeof Symbol < "u" && Symbol.species && y[Symbol.species] === y && Object.defineProperty(y, Symbol.species, { value: null, configurable: !0 })), y.alloc = function(l, f, k) {
            ...
            var l = 0 | this.length;
            return l === 0 ? "" : arguments.length === 0 ? ne(this, 0, l) : V.apply(this, arguments);
          ...
          }, y.prototype.compare = function(l, f, k, U, F) {
            if (!y.isBuffer(l)) throw new TypeError("Argument must be a Buffer");
          ...
          }, y.prototype.includes = function(l, f, k) {
            return this.indexOf(l, f, k) !== -1;
          }, y.prototype.indexOf = function(l, f, k) {
            return M(this, l, f, k, !0);
          }, y.prototype.lastIndexOf = function(l, f, k) {
            return M(this, l, f, k, !1);
          }, y.prototype.write = function(l, f, k, U) {
          ...
          function Pe(l, f, k, U, F) {
            return F || ke(l, 0, k, 4), _.write(l, f, k, U, 23, 4), k + 4;
          }
          function Se(l, f, k, U, F) {
            return F || ke(l, 0, k, 8), _.write(l, f, k, U, 52, 8), k + 8;
          }
          y.prototype.slice = function(l, f) {
            var k, U = this.length;
            if ((l = ~~l) < 0 ? (l += U) < 0 && (l = 0) : l > U && (l = U), (f = f === void 0 ? U : ~~f) < 0 ? (f += U) < 0 && (f = 0) : f > U && (f = U), f < l && (f = l), y.TYPED_ARRAY_SUPPORT) (k = this.subarray(l, f)).__proto__ = y.prototype;
            else {
              var F = f - l;
              k = new y(F, void 0);
            ...
            return k;
          }, y.prototype.readUIntLE = function(l, f, k) {
            l |= 0, f |= 0, k || D(l, f, this.length);
            for (var U = this[l], F = 1, H = 0; ++H < f && (F *= 256); ) U += this[l + H] * F;
            return U;
          }, y.prototype.readUIntBE = function(l, f, k) {
            l |= 0, f |= 0, k || D(l, f, this.length);
          ...
          }, y.prototype.readIntLE = function(l, f, k) {
            l |= 0, f |= 0, k || D(l, f, this.length);
            for (var U = this[l], F = 1, H = 0; ++H < f && (F *= 256); ) U += this[l + H] * F;
            return U >= (F *= 128) && (U -= Math.pow(2, 8 * f)), U;
          }, y.prototype.readIntBE = function(l, f, k) {
            l |= 0, f |= 0, k || D(l, f, this.length);
          ...
          }, y.prototype.writeUIntLE = function(l, f, k, U) {
            l = +l, f |= 0, k |= 0, U || ie(this, l, f, k, Math.pow(2, 8 * k) - 1, 0);
            var F = 1, H = 0;
            for (this[f] = 255 & l; ++H < k && (F *= 256); ) this[f + H] = l / F & 255;
            return f + k;
          }, y.prototype.writeUIntBE = function(l, f, k, U) {
            l = +l, f |= 0, k |= 0, U || ie(this, l, f, k, Math.pow(2, 8 * k) - 1, 0);
            var F = k - 1, H = 1;
            for (this[f + F] = 255 & l; --F >= 0 && (H *= 256); ) this[f + F] = l / H & 255;
            return f + k;
          }, y.prototype.writeUInt8 = function(l, f, k) {
            return l = +l, f |= 0, k || ie(this, l, f, 1, 255, 0), y.TYPED_ARRAY_SUPPORT || (l = Math.floor(l)), this[f] = 255 & l, f + 1;
          }, y.prototype.writeUInt16LE = function(l, f, k) {
            return l = +l, f |= 0, k || ie(this, l, f, 2, 65535, 0), y.TYPED_ARRAY_SUPPORT ? (this[f] = 255 & l, this[f + 1] = l >>> 8) : be(this, l, f, !0), f + 2;
          }, y.prototype.writeUInt16BE = function(l, f, k) {
            return l = +l, f |= 0, k || ie(this, l, f, 2, 65535, 0), y.TYPED_ARRAY_SUPPORT ? (this[f] = l >>> 8, this[f + 1] = 255 & l) : be(this, l, f, !1), f + 2;
          }, y.prototype.writeUInt32LE = function(l, f, k) {
            return l = +l, f |= 0, k || ie(this, l, f, 4, 4294967295, 0), y.TYPED_ARRAY_SUPPORT ? (this[f + 3] = l >>> 24, this[f + 2] = l >>> 16, this[f + 1] = l >>> 8, this[f] = 255 & l) : Te(this, l, f, !0), f + 4;
          }, y.prototype.writeUInt32BE = function(l, f, k) {
            return l = +l, f |= 0, k || ie(this, l, f, 4, 4294967295, 0), y.TYPED_ARRAY_SUPPORT ? (this[f] = l >>> 24, this[f + 1] = l >>> 16, this[f + 2] = l >>> 8, this[f + 3] = 255 & l) : Te(this, l, f, !1), f + 4;
          }, y.prototype.writeIntLE = function(l, f, k, U) {
            if (l = +l, f |= 0, !U) {
            ...
            return f + k;
          }, y.prototype.writeIntBE = function(l, f, k, U) {
            if (l = +l, f |= 0, !U) {
              var F = Math.pow(2, 8 * k - 1);
              ie(this, l, f, k, F - 1, -F);
            }
            ...
            return f + k;
          }, y.prototype.writeInt8 = function(l, f, k) {
            return l = +l, f |= 0, k || ie(this, l, f, 1, 127, -128), y.TYPED_ARRAY_SUPPORT || (l = Math.floor(l)), l < 0 && (l = 255 + l + 1), this[f] = 255 & l, f + 1;
          }, y.prototype.writeInt16LE = function(l, f, k) {
            return l = +l, f |= 0, k || ie(this, l, f, 2, 32767, -32768), y.TYPED_ARRAY_SUPPORT ? (this[f] = 255 & l, this[f + 1] = l >>> 8) : be(this, l, f, !0), f + 2;
          }, y.prototype.writeInt16BE = function(l, f, k) {
            return l = +l, f |= 0, k || ie(this, l, f, 2, 32767, -32768), y.TYPED_ARRAY_SUPPORT ? (this[f] = l >>> 8, this[f + 1] = 255 & l) : be(this, l, f, !1), f + 2;
          }, y.prototype.writeInt32LE = function(l, f, k) {
            return l = +l, f |= 0, k || ie(this, l, f, 4, 2147483647, -2147483648), y.TYPED_ARRAY_SUPPORT ? (this[f] = 255 & l, this[f + 1] = l >>> 8, this[f + 2] = l >>> 16, this[f + 3] = l >>> 24) : Te(this, l, f, !0), f + 4;
          }, y.prototype.writeInt32BE = function(l, f, k) {
            return l = +l, f |= 0, k || ie(this, l, f, 4, 2147483647, -2147483648), l < 0 && (l = 4294967295 + l + 1), y.TYPED_ARRAY_SUPPORT ? (this[f] = l >>> 24, this[f + 1] = l >>> 16, this[f + 2] = l >>> 8, this[f + 3] = 255 & l) : Te(this, l, f, !1), f + 4;
          }, y.prototype.writeFloatLE = function(l, f, k) {
            return Pe(this, l, f, !0, k);
          }, y.prototype.writeFloatBE = function(l, f, k) {
            return Pe(this, l, f, !1, k);
          }, y.prototype.writeDoubleLE = function(l, f, k) {
            return Se(this, l, f, !0, k);
          }, y.prototype.writeDoubleBE = function(l, f, k) {
            return Se(this, l, f, !1, k);
          }, y.prototype.copy = function(l, f, k, U) {
            ...
            else if (H < 1e3 || !y.TYPED_ARRAY_SUPPORT) for (F = 0; F < H; ++F) l[F + f] = this[F + k];
            else Uint8Array.prototype.set.call(l, this.subarray(k, k + H), f);
            return H;
          }, y.prototype.fill = function(l, f, k, U) {
            if (typeof l == "string") {
        ...
        for (var u = [], r = [], _ = typeof Uint8Array < "u" ? Uint8Array : Array, g = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", b = 0, v = g.length; b < v; ++b) u[b] = g[b], r[g.charCodeAt(b)] = b;
        function y(x) {
          var N = x.length;
        ...
        i.read = function(c, u, r, _, g) {
          var b, v, y = 8 * g - _ - 1, C = (1 << y) - 1, x = C >> 1, N = -7, G = r ? g - 1 : 0, K = r ? -1 : 1, j = c[u + G];
          for (G += K, b = j & (1 << -N) - 1, j >>= -N, N += y; N > 0; b = 256 * b + c[u + G], G += K, N -= 8) ;
          for (v = b & (1 << -N) - 1, b >>= -N, N += _; N > 0; v = 256 * v + c[u + G], G += K, N -= 8) ;
          if (b === 0) b = 1 - x;
          else {
        ...
        }, i.write = function(c, u, r, _, g, b) {
          var v, y, C, x = 8 * b - g - 1, N = (1 << x) - 1, G = N >> 1, K = g === 23 ? Math.pow(2, -24) - Math.pow(2, -77) : 0, j = _ ? 0 : b - 1, V = _ ? 1 : -1, P = u < 0 || u === 0 && 1 / u < 0 ? 1 : 0;
          for (u = Math.abs(u), isNaN(u) || u === 1 / 0 ? (y = isNaN(u) ? 1 : 0, v = N) : (v = Math.floor(Math.log(u) / Math.LN2), u * (C = Math.pow(2, -v)) < 1 && (v--, C *= 2), (u += v + G >= 1 ? K / C : K * Math.pow(2, 1 - G)) * C >= 2 && (v++, C /= 2), v + G >= N ? (y = 0, v = N) : v + G >= 1 ? (y = (u * C - 1) * Math.pow(2, g), v += G) : (y = u * Math.pow(2, G - 1) * Math.pow(2, g), v = 0)); g >= 8; c[r + j] = 255 & y, j += V, y /= 256, g -= 8) ;
          for (v = v << g | y, x += g; x > 0; c[r + j] = 255 & v, j += V, v /= 256, x -= 8) ;
          c[r + j - V] |= 128 * P;
        };
      }, function(m, i) {
        var c = {}.toString;
        m.exports = Array.isArray || function(u) {
          return c.call(u) == "[object Array]";
        };
      }, function(m, i, c) {
        Object.defineProperty(i, "__esModule", { value: !0 }), i.arrayToString = void 0, i.arrayToString = (u, r, _) => {
          const g = u.map(function(v, y) {
            const C = _(v, y);
...
`, j.getElementsByTagName("head")[0].appendChild(V)), j.body && G(j.body, "react-draggable-transparent-selection");
        ...
        Object.defineProperty(i, "__esModule", { value: !0 }), i.getBoundPosition = function(g, b, v) {
          ...
          var y = g.state, C = !(0, u.isNum)(y.lastX), x = _(g);
        ...
        function _(b, v) {
          ...
          return (u.test(y) && !b.disabled || y === "a" && b.href || v) && function(C) {
        ...
        var u, r = c(81), _ = (u = r) && u.__esModule ? u : { default: u }, g = c(36), b = null;
        function v(C, x) {
        ...
        var u = c(52), r = u.default, _ = u.DraggableCore;
        m.exports = r, m.exports.default = r, m.exports.DraggableCore = _;
      }, function(m, i, c) {
        var u = c(76), r = { "text/plain": "Text", "text/html": "Url", default: "Text" };
        m.exports = function(_, g) {
          var b, v, y, C, x, N, G = !1;
          g || (g = {}), b = g.debug || !1;
          try {
            if (y = u(), C = document.createRange(), x = document.getSelection(), (N = document.createElement("span")).textContent = _, N.style.all = "unset", N.style.position = "fixed", N.style.top = 0, N.style.clip = "rect(0, 0, 0, 0)", N.style.whiteSpace = "pre", N.style.webkitUserSelect = "text", N.style.MozUserSelect = "text", N.style.msUserSelect = "text", N.style.userSelect = "text", N.addEventListener("copy", function(K) {
        ...
        var u, r = c(77), _ = (u = r) && u.__esModule ? u : { default: u };
        i.default = _.default, m.exports = i.default;
      }, function(m, i, c) {
        m.exports = c(50);
      }, function(m, i, c) {
        var u = typeof Symbol == "function" && Symbol.for, r = u ? Symbol.for("react.element") : 60103, _ = u ? Symbol.for("react.portal") : 60106, g = u ? Symbol.for("react.fragment") : 60107, b = u ? Symbol.for("react.strict_mode") : 60108, v = u ? Symbol.for("react.profiler") : 60114, y = u ? Symbol.for("react.provider") : 60109, C = u ? Symbol.for("react.context") : 60110, x = u ? Symbol.for("react.async_mode") : 60111, N = u ? Symbol.for("react.concurrent_mode") : 60111, G = u ? Symbol.for("react.forward_ref") : 60112, K = u ? Symbol.for("react.suspense") : 60113, j = u ? Symbol.for("react.suspense_list") : 60120, V = u ? Symbol.for("react.memo") : 60115, P = u ? Symbol.for("react.lazy") : 60116, M = u ? Symbol.for("react.block") : 60121, B = u ? Symbol.for("react.fundamental") : 60117, I = u ? Symbol.for("react.responder") : 60118, J = u ? Symbol.for("react.scope") : 60119;
        function Q(A) {
        ...
        var u = 60103, r = 60106, _ = 60107, g = 60108, b = 60114, v = 60109, y = 60110, C = 60112, x = 60113, N = 60120, G = 60115, K = 60116, j = 60121, V = 60122, P = 60117, M = 60129, B = 60131;
        ...
        var Q = v, z = u, A = C, ae = _, le = K, ne = G, ce = r, ye = b, ee = g, de = x;
        i.ContextConsumer = y, i.ContextProvider = Q, i.Element = z, i.ForwardRef = A, i.Fragment = ae, i.Lazy = le, i.Memo = ne, i.Portal = ce, i.Profiler = ye, i.StrictMode = ee, i.Suspense = de, i.isAsyncMode = function() {
          ...
          return typeof D == "string" || typeof D == "function" || D === _ || D === b || D === M || D === g || D === x || D === N || D === B || typeof D == "object" && D !== null && (D.$$typeof === K || D.$$typeof === G || D.$$typeof === v || D.$$typeof === y || D.$$typeof === C || D.$$typeof === P || D.$$typeof === j || D[0] === V);
        }, i.typeOf = J;
      }, function(m, i, c) {
        ...
        var u = function(ee) {
          ...
          for (var be in ee) if (Object.prototype.hasOwnProperty.call(ee, be)) {
            ...
            Te && (Te.get || Te.set) ? Object.defineProperty(D, be, Te) : D[be] = ee[be];
          }
          return D.default = ee, de && de.set(ee, D), D;
        }(c(0)), r = N(c(18)), _ = N(c(12)), g = N(c(55)), b = c(32), v = c(40), y = c(20), C = N(c(57)), x = N(c(41));
        function N(ee) {
        ...
        function j() {
          return (j = Object.assign || function(ee) {
            for (var de = 1; de < arguments.length; de++) {
              var D = arguments[de];
              for (var ie in D) Object.prototype.hasOwnProperty.call(D, ie) && (ee[ie] = D[ie]);
            }
            return ee;
        ...
        function V(ee, de) {
          ...
          if (Object.getOwnPropertySymbols) {
            ...
            for (ie = 0; ie < Te.length; ie++) D = Te[ie], de.indexOf(D) >= 0 || Object.prototype.propertyIsEnumerable.call(ee, D) && (be[D] = ee[D]);
          }
          return be;
        }
        function P(ee, de) {
...
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
        ...
        function M(ee, de) {
          ...
          for (var D = 0, ie = new Array(de); D < de; D++) ie[D] = ee[D];
          return ie;
        }
        function B(ee, de) {
        ...
        function I(ee) {
          for (var de = 1; de < arguments.length; de++) {
            ...
            de % 2 ? B(Object(D), !0).forEach(function(ie) {
              ce(ee, ie, D[ie]);
            }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(ee, Object.getOwnPropertyDescriptors(D)) : B(Object(D)).forEach(function(ie) {
              Object.defineProperty(ee, ie, Object.getOwnPropertyDescriptor(D, ie));
            });
          }
          return ee;
        }
        ...
        function Q(ee, de, D) {
          return de && J(ee.prototype, de), D && J(ee, D), ee;
        }
        function z(ee, de) {
        ...
        function ce(ee, de, D) {
          return de in ee ? Object.defineProperty(ee, de, { value: D, enumerable: !0, configurable: !0, writable: !0 }) : ee[de] = D, ee;
        }
        var ye = function(ee) {
          (function(ie, be) {
          ...
          return Q(D, null, [{ key: "getDerivedStateFromProps", value: function(ie, be) {
            ...
            return u.createElement(C.default, j({}, Y, { onStart: this.onDragStart, onDrag: this.onDrag, onStop: this.onDragStop }), u.cloneElement(u.Children.only(Te), { className: F, style: I(I({}, Te.props.style), me), transform: l }));
          } }]), D;
        }(u.Component);
        ...
        _.resetWarningCache = r, m.exports = function() {
        ...
        (function() {
          ...
          m.exports ? (_.default = _, m.exports = _) : (u = (function() {
            return _;
          }).apply(i, [])) === void 0 || (m.exports = u);
        })();
      }, function(m, i, c) {
        ...
        function r() {
          ...
          if (b in v) return "";
          for (var y = 0; y < u.length; y++) if (_(b, u[y]) in v) return u[y];
          return "";
        }
        function _(b, v) {
        ...
        var u = function(ne) {
          ...
          for (var de in ne) if (Object.prototype.hasOwnProperty.call(ne, de)) {
            var D = ee ? Object.getOwnPropertyDescriptor(ne, de) : null;
            D && (D.get || D.set) ? Object.defineProperty(ye, de, D) : ye[de] = ne[de];
          }
          return ye.default = ne, ce && ce.set(ne, ye), ye;
        }(c(0)), r = C(c(18)), _ = C(c(12)), g = c(32), b = c(40), v = c(20), y = C(c(41));
        function C(ne) {
        ...
        function G(ne, ce) {
          return function(ye) {
            ...
            if (!(typeof Symbol > "u" || !(Symbol.iterator in Object(ye)))) {
              ...
              try {
                for (var Te, ke = ye[Symbol.iterator](); !(D = (Te = ke.next()).done) && (de.push(Te.value), !ee || de.length !== ee); D = !0) ;
              } catch (Pe) {
                ie = !0, be = Pe;
              } finally {
                try {
...
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
        ...
        var z = { start: "touchstart", move: "touchmove", stop: "touchend" }, A = { start: "mousedown", move: "mousemove", stop: "mouseup" }, ae = A, le = function(ne) {
          ...
          function de() {
            ...
            return Q(I(D = ee.call.apply(ee, [this].concat(be))), "state", { dragging: !1, lastX: NaN, lastY: NaN, touchIdentifier: null }), Q(I(D), "mounted", !1), Q(I(D), "handleDragStart", function(ke) {
              ...
              if (!(D.props.disabled || !(ke.target instanceof Se.defaultView.Node) || D.props.handle && !(0, g.matchesSelectorAndParentsTo)(ke.target, D.props.handle, Pe) || D.props.cancel && (0, g.matchesSelectorAndParentsTo)(ke.target, D.props.cancel, Pe))) {
                ...
                if (Je != null) {
                  ...
                  (0, y.default)("DraggableCore: handleDragStart: %j", me), (0, y.default)("calling", D.props.onStart), D.props.onStart(ke, me) !== !1 && D.mounted !== !1 && (D.props.enableUserSelectHack && (0, g.addUserSelectStyles)(Se), D.setState({ dragging: !0, lastX: X, lastY: Y }), (0, g.addEvent)(Se, ae.move, D.handleDrag), (0, g.addEvent)(Se, ae.stop, D.handleDragStop));
                }
              }
              ...
              if (Pe != null) {
                ...
                if ((0, y.default)("DraggableCore: handleDrag: %j", me), D.props.onDrag(ke, me) !== !1 && D.mounted !== !1) D.setState({ lastX: Se, lastY: ze });
                else try {
              ...
              if (D.state.dragging) {
                ...
                if (Pe != null) {
                  ...
                  var X = D.findDOMNode();
                  X && D.props.enableUserSelectHack && (0, g.removeUserSelectStyles)(X.ownerDocument), (0, y.default)("DraggableCore: handleDragStop: %j", Je), D.setState({ dragging: !1, lastX: NaN, lastY: NaN }), X && ((0, y.default)("DraggableCore: Removing handlers"), (0, g.removeEvent)(X.ownerDocument, ae.move, D.handleDrag), (0, g.removeEvent)(X.ownerDocument, ae.stop, D.handleDragStop));
                }
              }
            }), Q(I(D), "onMouseDown", function(ke) {
              return ae = A, D.handleDragStart(ke);
            }), Q(I(D), "onMouseUp", function(ke) {
              return ae = A, D.handleDragStop(ke);
            }), Q(I(D), "onTouchStart", function(ke) {
              return ae = z, D.handleDragStart(ke);
            }), Q(I(D), "onTouchEnd", function(ke) {
              return ae = z, D.handleDragStop(ke);
            }), D;
          }
        ...
        (m.exports = c(7)(!1)).push([m.i, ".ck-inspector-side-pane{position:relative}", ""]);
      }, function(m, i, c) {
        var u = c(6), r = c(69);
        typeof (r = r.__esModule ? r.default : r) == "string" && (r = [[m.i, r, ""]]);
        var _ = { injectType: "singletonStyleTag", attributes: { "data-cke-inspector": !0 }, insert: "head", singleton: !0 };
        ...
        (m.exports = c(7)(!1)).push([m.i, ".ck-inspector .ck-inspector-checkbox{vertical-align:middle}", ""]);
      }, function(m, i, c) {
        var u = c(6), r = c(71);
        typeof (r = r.__esModule ? r.default : r) == "string" && (r = [[m.i, r, ""]]);
        var _ = { injectType: "singletonStyleTag", attributes: { "data-cke-inspector": !0 }, insert: "head", singleton: !0 };
        ...
        (m.exports = c(7)(!1)).push([m.i, ".ck-inspector-model-tree__hide-markers .ck-inspector-tree__position.ck-inspector-tree__position_marker{display:none}", ""]);
      }, function(m, i) {
        m.exports = function() {
          var c = document.getSelection();
          if (!c.rangeCount) return function() {
          };
          for (var u = document.activeElement, r = [], _ = 0; _ < c.rangeCount; _++) r.push(c.getRangeAt(_));
          ...
          return c.removeAllRanges(), function() {
            c.type === "Caret" && c.removeAllRanges(), c.rangeCount || r.forEach(function(g) {
              c.addRange(g);
            }), u && u.focus();
          };
        };
        ...
        var z = function(A) {
          function ae() {
            ...
            return ne = ce = V(this, (le = ae.__proto__ || Object.getPrototypeOf(ae)).call.apply(le, [this].concat(ee))), ce.removePortal = function() {
              ...
              var ie = J()(ce, g.default.createElement(y.default, u({ defaultStyles: ae.defaultStyles }, D)), ce.node);
              ce.portalRef(ie);
            }, V(ce, ne);
          }
          return function(le, ne) {
            ...
            if (x.canUseDOM) {
              ...
              ie !== D && (D.removeChild(this.node), ie.appendChild(this.node)), (le.isOpen || ee) && !B && this.renderPortal(this.props);
            }
          } }, { key: "componentWillUnmount", value: function() {
            ...
            return x.canUseDOM && B ? (!this.node && B && (this.node = I("div")), J()(g.default.createElement(y.default, u({ ref: this.portalRef, defaultStyles: ae.defaultStyles }, this.props)), this.node)) : null;
        ...
        }(), g = c(0), b = V(c(18)), v = j(c(79)), y = V(c(80)), C = j(c(43)), x = j(c(83)), N = c(36), G = V(N), K = V(c(44));
        function j(I) {
        ...
        var P = { overlay: "ReactModal__Overlay", content: "ReactModal__Content" }, M = 0, B = function(I) {
          ...
          return function(Q, z) {
            ...
            return this.props.overlayElement(ee, D);
          } }]), J;
        }(g.Component);
        ...
        var u, r = c(42), _ = (u = r) && u.__esModule ? u : { default: u }, g = [], b = null, v = !1;
        function y() {
          v = !0;
        }
        function C() {
        ...
        var u, r = c(42), _ = (u = r) && u.__esModule ? u : { default: u };
        m.exports = i.default;
      }, function(m, i, c) {
        var u = function() {
        };
        ...
        (function() {
          ...
          (u = (function() {
            return _;
          }).call(i, c, i, m)) === void 0 || (m.exports = u);
        })();
      }, function(m, i, c) {
        Object.defineProperty(i, "__esModule", { value: !0 }), i.resetState = function() {
          var g = document.getElementsByTagName("html")[0];
          for (var b in u) _(g, u[b]);
          var v = document.body;
        ...
        i.add = function(g, b) {
          return v = g.classList, y = g.nodeName.toLowerCase() == "html" ? u : r, void b.split(" ").forEach(function(C) {
            (function(x, N) {
              x[N] || (x[N] = 0), x[N] += 1;
            })(y, C), v.add(C);
          });
        ...
        var u, r = c(44), _ = (u = r) && u.__esModule ? u : { default: u }, g = void 0, b = void 0, v = [];
        function y() {
...
https://fb.me/react-async-component-lifecycle-hooks`);
          ...
          if (typeof b.getDerivedStateFromProps == "function" && (v.componentWillMount = u, v.componentWillReceiveProps = r), typeof v.getSnapshotBeforeUpdate == "function") {
        ...
        (m.exports = c(7)(!1)).push([m.i, ".ck-inspector .ck-inspector-editor-quick-actions{display:flex;align-content:center;justify-content:center;align-items:center;flex-direction:row;flex-wrap:nowrap}.ck-inspector .ck-inspector-editor-quick-actions>.ck-inspector-button{margin-left:.3em}.ck-inspector .ck-inspector-editor-quick-actions>.ck-inspector-button.ck-inspector-button_data-copied{animation-duration:.5s;animation-name:ck-inspector-bounce-in;color:green}@keyframes ck-inspector-bounce-in{0%{opacity:0;transform:scale3d(.5,.5,.5)}20%{transform:scale3d(1.1,1.1,1.1)}40%{transform:scale3d(.8,.8,.8)}60%{opacity:1;transform:scale3d(1.05,1.05,1.05)}to{opacity:1;transform:scaleX(1)}}", ""]);
      }, function(m, i, c) {
        var u = c(6), r = c(91);
        typeof (r = r.__esModule ? r.default : r) == "string" && (r = [[m.i, r, ""]]);
        var _ = { injectType: "singletonStyleTag", attributes: { "data-cke-inspector": !0 }, insert: "head", singleton: !0 };
        ...
        (m.exports = c(7)(!1)).push([m.i, "html body.ck-inspector-body-expanded{margin-bottom:var(--ck-inspector-height)}html body.ck-inspector-body-collapsed{margin-bottom:var(--ck-inspector-collapsed-height)}.ck-inspector-wrapper *{box-sizing:border-box}", ""]);
      }, , , function(m, i, c) {
        c.r(i), c.d(i, "default", function() {
          return $e;
        });
        var u = c(0), r = c.n(u), _ = c(12), g = c.n(_);
        function b(E) {
        ...
        function N(E, s, d) {
          ...
          return Be({ type: C.INIT }), (h = { dispatch: Be, subscribe: Ne, getState: Ee, replaceReducer: Ie })[v] = Ke, h;
        }
        ...
        var M = typeof window < "u" && window.document !== void 0 && window.document.createElement !== void 0 ? u.useLayoutEffect : u.useEffect, B = function(E) {
          var s = E.store, d = E.context, h = E.children, S = Object(u.useMemo)(function() {
            var re = P(s);
          ...
          var L = d || G;
          return r.a.createElement(L.Provider, { value: S }, h);
        };
        function I() {
        ...
        var Q = c(39), z = c.n(Q), A = c(45), ae = ["getDisplayName", "methodName", "renderCountProp", "shouldHandleStateChanges", "storeKey", "withRef", "forwardRef", "context"], le = ["reactReduxForwardedRef"], ne = [], ce = [null, null];
        function ye(E, s) {
          var d = E[1];
        ...
        function D(E, s, d, h, S, O, L, re, fe, pe) {
          if (E) {
        ...
        function be(E, s) {
          ...
          } : h, O = d.methodName, L = O === void 0 ? "connectAdvanced" : O, re = d.renderCountProp, fe = re === void 0 ? void 0 : re, pe = d.shouldHandleStateChanges, Ee = pe === void 0 || pe, Ne = d.storeKey, Be = Ne === void 0 ? "store" : Ne, Ie = (d.withRef, d.forwardRef), Ke = Ie !== void 0 && Ie, Me = d.context, Fe = Me === void 0 ? G : Me, et = J(d, ae), Ye = Fe;
          return function(Le) {
            ...
            function zt(lt) {
              var An = Object(u.useMemo)(function() {
                ...
                return Wn && Wn.Consumer && Object(A.isContextConsumer)(r.a.createElement(Wn.Consumer, null)) ? Wn : Ye;
              }, [Wn, Ye]), Mn = Object(u.useContext)(ao), $t = !!lt.store && !!lt.store.getState && !!lt.store.dispatch;
              Mn && Mn.store;
              ...
              ee(de, [gr, Bi, li, xt, Zn, jo, Lr]), ee(D, [Ee, dn, fn, Xn, gr, Bi, li, jo, Lr, mr], [dn, fn, Xn]);
        ...
        var me = [function(E) {
          return typeof E == "function" ? /* @__PURE__ */ function(s) {
            return function(d, h) {
              h.displayName;
              var S, O = h.pure, L = h.areMergedPropsEqual, re = !1;
        ...
        function k(E, s, d, h, S) {
          var O, L, re, fe, pe, Ee = S.areStatesEqual, Ne = S.areOwnPropsEqual, Be = S.areStatePropsEqual, Ie = !1;
          function Ke(Me, Fe) {
            var et, Ye, Le = !Ne(Fe, L), dt = !Ee(Me, O);
            return O = Me, L = Fe, Le && dt ? (re = E(O, L), s.dependsOnOwnProps && (fe = s(h, L)), pe = d(re, fe, L)) : Le ? (E.dependsOnOwnProps && (re = E(O, L)), s.dependsOnOwnProps && (fe = s(h, L)), pe = d(re, fe, L)) : (dt && (et = E(O, L), Ye = !Be(et, re), re = et, Ye && (pe = d(re, fe, L))), pe);
          }
          return function(Me, Fe) {
            return Ie ? Ke(Me, Fe) : (re = E(O = Me, L = Fe), fe = s(h, L), pe = d(re, fe, L), Ie = !0, pe);
          };
        }
        function U(E, s) {
          var d = s.initMapStateToProps, h = s.initMapDispatchToProps, S = s.initMergeProps, O = J(s, l), L = d(E, O), re = h(E, O), fe = S(E, O);
        ...
        function je(E) {
          var s = {}, d = s.connectHOC, h = d === void 0 ? be : d, S = s.mapStateToPropsFactories, O = S === void 0 ? X : S, L = s.mapDispatchToPropsFactories, re = L === void 0 ? Je : L, fe = s.mergePropsFactories, pe = fe === void 0 ? me : fe, Ee = s.selectorFactory, Ne = Ee === void 0 ? U : Ee;
          return function(Be, Ie, Ke, Me) {
            Me === void 0 && (Me = {});
            var Fe = Me, et = Fe.pure, Ye = et === void 0 || et, Le = Fe.areStatesEqual, dt = Le === void 0 ? he : Le, Dt = Fe.areOwnPropsEqual, Mt = Dt === void 0 ? ke : Dt, jt = Fe.areStatePropsEqual, _t = jt === void 0 ? ke : jt, zt = Fe.areMergedPropsEqual, ft = zt === void 0 ? ke : zt, In = J(Fe, F), lt = H(Be, O, "mapStateToProps"), An = H(Ie, re, "mapDispatchToProps"), Wn = H(Ke, pe, "mergeProps");
            return h(Ne, I({ methodName: "connect", getDisplayName: function(zr) {
              return "Connect(" + zr + ")";
            }, shouldHandleStateChanges: !!Be, initMapStateToProps: lt, initMapDispatchToProps: An, initMergeProps: Wn, pure: Ye, areStatesEqual: dt, areOwnPropsEqual: Mt, areStatePropsEqual: _t, areMergedPropsEqual: ft }, In));
          };
        }
        var Re = je(), Xe;
        ...
        function or(E, s, d) {
          const h = Tt(E), S = { ...s, ...d }, O = S.currentRootName, L = Object(gt.c)(h, O), re = Object(gt.a)(h, O), fe = Object(gt.e)({ currentEditor: h, currentRootName: S.currentRootName, ranges: L, markers: re });
          let pe = S.currentNode, Ee = S.currentNodeDefinition;
          return pe ? pe.root.rootName !== O || !Object(cn.d)(pe) && !pe.parent ? (pe = null, Ee = null) : Ee = Object(gt.b)(h, pe) : Ee = null, { treeDefinition: fe, currentNode: pe, currentNodeDefinition: Ee, ranges: L, markers: re };
        }
        function _r(E) {
        ...
        function gn(E, s, d) {
          ...
          let re = h.currentNode, fe = h.currentNodeDefinition;
          return re ? re.root.rootName !== S || !Object(Yt.g)(re) && !re.parent ? (re = null, fe = null) : fe = Object(en.b)(re) : fe = null, { treeDefinition: L, currentNode: re, currentNodeDefinition: fe, ranges: O };
        }
        function Sr() {
        ...
        function sr({ editors: E, currentEditorName: s }, d) {
          if (!d) return null;
          const h = E.get(s).model.schema, S = h.getDefinitions()[d], O = {}, L = {}, re = {};
          let fe = {};
        ...
        var W = c(46), se = c.n(W), we = /* @__PURE__ */ function() {
          var E = function(s, d) {
        ...
        }(u.PureComponent), st = c(14), tt = c.n(st), Ot = /* @__PURE__ */ function() {
          var E = function(s, d) {
            return (E = Object.setPrototypeOf || { __proto__: [] } instanceof Array && function(h, S) {
              h.__proto__ = S;
            } || function(h, S) {
          ...
          d === void 0 && (d = 0);
          var h = s.reduce(function(O, L, re) {
          ...
          return Ot(s, E), Object.defineProperty(s.prototype, "parentNode", { get: function() {
            ...
            return ht("right", re) && (Ie = fe.width + (d - fe.x) * O / S, Ee && (Ke = (Ie - Fe) / this.ratio + Me)), ht("left", re) && (Ie = fe.width - (d - fe.x) * O / S, Ee && (Ke = (Ie - Fe) / this.ratio + Me)), ht("bottom", re) && (Ke = fe.height + (h - fe.y) * O / S, Ee && (Ie = (Ke - Me) * this.ratio + Fe)), ht("top", re) && (Ke = fe.height - (h - fe.y) * O / S, Ee && (Ie = (Ke - Me) * this.ratio + Fe)), { newWidth: Ie, newHeight: Ke };
          }, s.prototype.calculateNewSizeFromAspectRatio = function(d, h, S, O) {
            var L = this.props, re = L.lockAspectRatio, fe = L.lockAspectRatioExtraHeight, pe = L.lockAspectRatioExtraWidth, Ee = O.width === void 0 ? 10 : O.width, Ne = S.width === void 0 || S.width < 0 ? d : S.width, Be = O.height === void 0 ? 10 : O.height, Ie = S.height === void 0 || S.height < 0 ? h : S.height, Ke = fe || 0, Me = pe || 0;
            if (re) {
              var Fe = (Be - Ke) * this.ratio + Me, et = (Ie - Ke) * this.ratio + Me, Ye = (Ee - Me) / this.ratio + Ke, Le = (Ne - Me) / this.ratio + Ke, dt = Math.max(Ee, Fe), Dt = Math.min(Ne, et), Mt = Math.max(Be, Ye), jt = Math.min(Ie, Le);
              d = Pt(d, dt, Dt), h = Pt(h, Mt, jt);
            } else d = Pt(d, Ee, Ne), h = Pt(h, Be, Ie);
            ...
            if (this.state.isResizing && this.resizable && this.window) {
              ...
              var h = this.props, S = h.maxWidth, O = h.maxHeight, L = h.minWidth, re = h.minHeight, fe = Bt(d) ? d.touches[0].clientX : d.clientX, pe = Bt(d) ? d.touches[0].clientY : d.clientY, Ee = this.state, Ne = Ee.direction, Be = Ee.original, Ie = Ee.width, Ke = Ee.height, Me = this.getParentSize(), Fe = Nn(Me, this.window.innerWidth, this.window.innerHeight, S, O, L, re);
              S = Fe.maxWidth, O = Fe.maxHeight, L = Fe.minWidth, re = Fe.minHeight;
              var et = this.calculateNewSizeFromDirection(fe, pe), Ye = et.newHeight, Le = et.newWidth, dt = this.calculateNewMaxFromBoundary(S, O), Dt = this.calculateNewSizeFromAspectRatio(Le, Ye, { width: dt.maxWidth, height: dt.maxHeight }, { width: L, height: re });
              if (Le = Dt.newWidth, Ye = Dt.newHeight, this.props.grid) {
                var Mt = tn(Le, this.props.grid[0]), jt = tn(Ye, this.props.grid[1]), _t = this.props.snapGap || 0;
                Le = _t === 0 || Math.abs(Mt - Le) <= _t ? Mt : Le, Ye = _t === 0 || Math.abs(jt - Ye) <= _t ? jt : Ye;
              }
            ...
            var d = this, h = this.props, S = h.enable, O = h.handleStyles, L = h.handleClasses, re = h.handleWrapperStyle, fe = h.handleWrapperClass, pe = h.handleComponent;
            if (!S) return null;
            var Ee = Object.keys(S).map(function(Ne) {
              return S[Ne] !== !1 ? u.createElement(qe, { key: Ne, direction: Ne, onResizeStart: d.onResizeStart, replaceStyles: O && O[Ne], className: L && L[Ne] }, pe && pe[Ne] ? pe[Ne] : null) : null;
            });
            return u.createElement("div", { className: fe, style: re }, Ee);
          }, s.prototype.render = function() {
            var d = this, h = Object.keys(this.props).reduce(function(L, re) {
            ...
            return u.createElement(O, nt({ ref: this.ref, style: S, className: this.props.className }, h), this.state.isResizing && u.createElement("div", { style: this.state.backgroundStyle }), this.props.children, this.renderResizer());
        ...
        }, qn = se.a, Kt = { width: "auto", height: "auto", display: "inline-block", position: "absolute", top: 0, left: 0 }, Or = function(E) {
          function s(d) {
            var h = E.call(this, d) || this;
          ...
          return function(d, h) {
            ...
            var d = this.offsetFromParent, h = d.left, S = d.top, O = this.getDraggablePosition(), L = O.x, re = O.y;
            this.draggable.setState({ x: L - h, y: re - S }), this.forceUpdate();
            ...
            if (this.resizingPosition = { x: re.x + L.left, y: re.y + L.top }, this.setState({ original: re }), this.props.bounds) {
              ...
              if (Ee instanceof Element && (pe instanceof HTMLElement || pe === window) && fe instanceof HTMLElement) {
                ...
                var Fe = Ee.getBoundingClientRect(), et = Fe.left, Ye = Fe.top, Le = this.props.bounds === "window" ? { left: 0, top: 0 } : pe.getBoundingClientRect(), dt = Le.left, Dt = Le.top, Mt = this.getOffsetWidth(pe), jt = this.getOffsetHeight(pe), _t = h.toLowerCase().endsWith("left"), zt = h.toLowerCase().endsWith("right"), ft = h.startsWith("top"), In = h.startsWith("bottom");
                if (_t && this.resizable) {
                  var lt = (et - dt) / O + this.resizable.size.width;
                  this.setState({ maxWidth: lt > Number(Be) ? Be : lt });
                }
                (zt || this.props.lockAspectRatio && !_t) && (lt = Mt + (dt - et) / O, this.setState({ maxWidth: lt > Number(Be) ? Be : lt })), ft && this.resizable && (lt = (Ye - Dt) / O + this.resizable.size.height, this.setState({ maxHeight: lt > Number(Ie) ? Ie : lt })), (In || this.props.lockAspectRatio && !ft) && (lt = jt + (Dt - Ye) / O, this.setState({ maxHeight: lt > Number(Ie) ? Ie : lt }));
              }
            ...
            ["top", "left", "topLeft", "bottomLeft", "topRight"].indexOf(h) !== -1 && (h === "bottomLeft" ? L.x += re : (h === "topRight" || (L.x += re), L.y += fe)), L.x === this.draggable.state.x && L.y === this.draggable.state.y || this.draggable.setState(L), this.updateOffsetFromParent();
            ...
            this.setState({ maxWidth: re, maxHeight: fe }), this.props.onResizeStop && this.props.onResizeStop(d, h, S, O, this.resizingPosition);
          }, s.prototype.updateSize = function(d) {
        ...
        class fr extends u.Component {
          render() {
            ...
            for (const h in this.props.itemDefinitions) {
              const S = this.props.itemDefinitions[h], { subProperties: O, presentation: L = {} } = S, re = O && Object.keys(O).length, fe = Object(ut.c)(String(S.value), 2e3), pe = [r.a.createElement(Jr, { key: `${this.props.name}-${h}-name`, name: h, listUid: this.props.name, canCollapse: re, colorBox: L.colorBox, expandCollapsibles: s, onClick: this.props.onPropertyTitleClick, title: S.title }), r.a.createElement("dd", { key: `${this.props.name}-${h}-value` }, r.a.createElement("input", { id: `${this.props.name}-${h}-value-input`, type: "text", value: fe, readOnly: !0 }))];
              re && pe.push(r.a.createElement(fr, { name: `${this.props.name}-${h}`, key: `${this.props.name}-${h}`, itemDefinitions: O, presentation: this.props.presentation })), d.push(pe);
            }
        ...
        var To = Re(({ editors: E, currentEditorName: s, view: { treeDefinition: d, currentRootName: h, currentNode: S, ui: { showElementTypes: O } } }) => ({ treeDefinition: d, editors: E, currentEditorName: s, currentRootName: h, currentNode: S, showElementTypes: O }), { setViewCurrentRootName: function(E) {
        ...
        var Oo = Re(({ editors: E, currentEditorName: s, view: { ranges: d } }) => {
          const h = E.get(s);
          return { editor: h, currentEditorName: s, info: function(S, O) {
            const L = S.editing.view.document.selection, re = { properties: { isCollapsed: { value: L.isCollapsed }, isBackward: { value: L.isBackward }, isFake: { value: L.isFake }, rangeCount: { value: L.rangeCount } }, anchor: Ir(Object(Yt.a)(L.anchor)), focus: Ir(Object(Yt.a)(L.focus)), ranges: {} };
            O.forEach((fe, pe) => {
        ...
        function Ir({ offset: E, isAtEnd: s, isAtStart: d, parent: h }) {
        ...
        var Di = Re(({ editors: E, currentEditorName: s, commands: { currentCommandName: d, currentCommandDefinition: h } }) => ({ editors: E, currentEditorName: s, currentCommandName: d, currentCommandDefinition: h }), {})(Zo);
        ...
        var Ai = Re(({ editors: E, currentEditorName: s, schema: { currentSchemaDefinitionName: d, currentSchemaDefinition: h } }) => ({ editors: E, currentEditorName: s, currentSchemaDefinitionName: d, currentSchemaDefinition: h }), { setSchemaCurrentDefinitionName: ir })(Ii);
        class ei extends u.Component {
          render() {
            return this.props.currentEditorName ? r.a.createElement(Et, { splitVertically: "true" }, r.a.createElement(Ri, null), r.a.createElement(vn, null, r.a.createElement(Qt, { activeTab: "Inspect" }, r.a.createElement(Ai, { label: "Inspect" })))) : r.a.createElement(Et, { isEmpty: "true" }, r.a.createElement("p", null, "Nothing to show. Attach another editor instance to start inspecting."));
          }
        }
        var ti = Re(({ currentEditorName: E }) => ({ currentEditorName: E }))(ei), ni = c(47), Mi = c.n(ni), ri = c(48), oi = c.n(ri);
        function ji() {
        ...
        var ai = Re(({ editors: E, currentEditorName: s, ui: { isCollapsed: d, height: h, activeTab: S } }) => ({ isCollapsed: d, height: h, editors: E, currentEditorName: s, activeTab: S }), { toggleIsCollapsed: It, setHeight: function(E) {
        ...
        c(90), window.CKEDITOR_INSPECTOR_VERSION = "4.1.0";
        class $e {
          constructor() {
          ...
          static _mount(s) {
            ...
            }), g.a.render(r.a.createElement(B, { store: $e._store }, r.a.createElement(ai, null)), d);
          }
...
const Cu = function(xe) {
  const R = xe.plugins.get(tc), m = $(xe.ui.view.element), i = $(xe.sourceElement), c = `ckeditor${Math.floor(Math.random() * 1e9)}`, u = [
    "keypress",
    "keyup",
    "change",
    "focus",
  ...
  let m = null;
  const i = xe.editing.view.document, c = xe.plugins.get("ClipboardPipeline");
  i.on("clipboardOutput", (u, r) => {
    m = xe.id;
  }), i.on("clipboardInput", async (u, r) => {
    let _ = r.dataTransfer.getData("text/html");
    if (_ && _.includes("<craft-entry") && !(r.method == "drop" && m === xe.id)) {
      if (r.method == "paste" || r.method == "drop" && m !== xe.id) {
        let g = _, b = !1;
        ...
        for (let P = 0; P < N.length; P++) {
          ...
          if (N[P][1] && (M = N[P][1]), M !== null) {
            ...
            if (!(m === xe.id && !B.test(x))) {
              let I = null;
              m !== xe.id && (R.includes(gu) ? I = xe.config.get("entryTypeOptions").map((J) => J.value) : (Craft.cp.displayError(
                Craft.t(
                  "ckeditor",
        ...
        b || (r.content = xe.data.htmlProcessor.toView(g), c.fire("inputTransformation", r));
      }
    }
  });
}, Au = async function(xe, R) {
  typeof xe == "string" && (xe = document.querySelector(`#${xe}`)), R.licenseKey = "GPL";
  const m = await ru.create(xe, R);
  return Craft.showCkeditorInspector && Craft.userIsAdmin && Su.attach(m), m.editing.view.change((i) => {
    const c = m.editing.view.document.getRoot();
    if (typeof R.accessibleFieldName < "u" && R.accessibleFieldName.length) {
      let u = c.getAttribute("aria-label");
      i.setAttribute(
        "aria-label",
        R.accessibleFieldName + ", " + u,
        c
      );
    }
  ...
  }), R.plugins.includes(tc) && Cu(m), R.plugins.includes(ou) && Tu(m, R), Ou(m, R.plugins), m;
};
export {
  gu as CraftEntries,
